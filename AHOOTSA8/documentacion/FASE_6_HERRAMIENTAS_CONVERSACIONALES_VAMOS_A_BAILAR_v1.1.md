...
**Estado de este documento:** implementación funcional v1.1  
...
## 0. Corrección v1.1 — archivo funcional incluido

La primera entrega de la Fase 6 contenía únicamente el diseño y el manual.
Faltaba el archivo ejecutable:

```text
external_content\external_tools\ahootsa_dances.py
```

La versión v1.1 ya lo incluye.

El módulo define y registra:
...
También se incluyen archivos auxiliares que deben fusionarse manualmente:

```text
external_content\external_profiles\ahootsa\
├── FASE_6_ANADIR_A_TOOLS.txt
└── FASE_6_ANADIR_A_INSTRUCTIONS.txt

FASE_6_ANADIR_A_ENV.txt
```

Estos archivos no sustituyen automáticamente el perfil existente para evitar
perder las herramientas e instrucciones de las fases anteriores.
...
La herramienta usa el sistema multimedia disponible en:
...
```python
deps.reachy_mini.media.play_sound(ruta_audio)
```

En `reachy-mini 1.9.0`, `MediaManager` no expone un método público
`stop_sound()`. La implementación v1.1 detiene primero el `playbin` de sonido
del backend LOCAL para no interrumpir la canalización de voz. Como respaldo,
utiliza `stop_playing()` y vuelve a ejecutar `start_playing()`.
...
## 26. Instalación de la implementación v1.1

Extraer el ZIP funcional sobre:
...
Reiniciar después la aplicación conversacional y comprobar en los logs:
