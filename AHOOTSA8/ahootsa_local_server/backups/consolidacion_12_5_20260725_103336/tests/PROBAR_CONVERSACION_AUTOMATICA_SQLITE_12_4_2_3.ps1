...
function Get-AhootsaEventIds {
    ...
    while ($pending.Count -gt 0) {
        ...
        if ($null -eq $idProperty) {
            throw (
                "La respuesta del listado contiene un elemento sin propiedad id. " +
                "Tipo: {0}" -f $current.GetType().FullName
            )
        }

        foreach ($idValue in @($idProperty.Value)) {
            $result.Add([int]$idValue)
        }
    }
...
Write-Host "PRUEBA 12.4.2.3: CONVERSACION AUTOMATICA, SQLITE Y LISTADO API" -ForegroundColor Cyan
...
try {
    $prepared = Invoke-AhootsaJson -Uri "$BaseUrl/panel/api/session/prepare" -Method POST -Body @{
        ...
        level=[string]$definition.session.level; started_by="Prueba automatica 12.4.2.3"
    }
    ...
    foreach ($item in @($definition.events)) {
        ...
        $metadata["test_name"] = "conversacion_automatica_12_4_2_3"
    ...
    $apiResponse = Invoke-AhootsaJson -Uri "$BaseUrl/sessions/$sessionId/events"
    $apiIds = @(Get-AhootsaEventIds -Response $apiResponse)
    ...
    $finished = Invoke-AhootsaJson -Uri "$BaseUrl/panel/api/session/finish" -Method POST -Body @{
        note=("Prueba automatica 12.4.2.3 completada. Test run: {0}" -f $runId); decision="no_decision"
    }
    $finishedOk = $true
    ...
    $reportPath = Join-Path $sessionDirectory "automatic_conversation_verification_12_4_2_3.json"
    @{
        test_name="conversacion_automatica_12_4_2_3"; test_run_id=$runId; verified_at=(Get-Date).ToUniversalTime().ToString("o");
    ...
    Write-Host ""
    Write-Host "PRUEBA 12.4.2.3 VALIDADA: SQLITE Y LISTADO API COMPROBADOS." -ForegroundColor Green
    ...
    if ($null -ne $prepared -and -not $finishedOk) {
        try { Invoke-AhootsaJson -Uri "$BaseUrl/panel/api/session/finish" -Method POST -Body @{ note=("Prueba 12.4.2.3 abortada: {0}" -f $_.Exception.Message); decision="no_decision" } | Out-Null } catch {}
    }
    throw
}
