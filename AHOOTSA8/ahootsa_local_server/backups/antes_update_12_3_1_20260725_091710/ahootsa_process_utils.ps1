...
function Stop-AhootsaPortProcess {
    ...
    foreach ($processId in $processIds) {
        ...
        try {
            ...
            Write-Host (
                "$ServiceName: stopping previous process PID " +
            ...
            Write-Host (
                "$ServiceName: could not stop PID $processId: " +
                $_.Exception.Message
            ) -ForegroundColor Yellow
        }
    }
    ...
    Write-Host "$ServiceName: previous listener removed." -ForegroundColor Green
    return $true
}
...
function Stop-AhootsaCommandProcesses {
    ...
    foreach ($process in $processes) {
        ...
        try {
            Write-Host (
                "$ServiceName: stopping stale process PID $processId..."
            ) -ForegroundColor Yellow
            Stop-Process -Id $processId -Force -ErrorAction Stop
        } catch {
            Write-Host (
                "$ServiceName: could not stop stale PID $processId."
            ) -ForegroundColor Yellow
        }
    }
}
