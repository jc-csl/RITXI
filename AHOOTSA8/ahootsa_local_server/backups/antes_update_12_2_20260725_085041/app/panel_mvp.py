...
def _panel_summary(session: models.SessionRecord) -> dict[str, Any]:
    ...
    duration = max((effective_end - session.started_at).total_seconds(), 0.0)
...
def panel_bootstrap(db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    if active_summary is not None:
        try:
            runtime_paths = session_preparation_service.ensure_runtime_structure()
            launcher = (
                runtime_paths["sessions"]
                / f"session_{active.id:06d}"
                / "2_lanzar_app_ahootsa_sesion.ps1"
            )
            active_summary["runtime_prepared"] = launcher.exists()
        except RuntimeError:
            active_summary["runtime_prepared"] = False
...
def finish_panel_session(
    ...
    try:
        paths = session_preparation_service.ensure_runtime_structure()
        status_path = (
            paths["sessions"]
            / f"session_{session.id:06d}"
            / "session_status.json"
        )
            ...
            status.update(
                {
                    ...
                    "professional_decision": payload.decision,
                }
            )
        ...
        # El cierre de base de datos es prioritario. Un problema de diagnóstico
        # no debe reabrir la sesión.
        pass
