...
"""Ahootsa 7.0.23 panel/API routes. Terminal más limpio y salida desde panel.
...
class _AhootsaAccessLogFilter(logging.Filter):
    """Reduce ruido de terminal sin ocultar errores ni llamadas de herramientas.

    Uvicorn imprime una línea por cada petición HTTP. El panel consulta
    /memory/state periódicamente para actualizar las cartas; registrar cada
    consulta produce decenas de líneas repetidas y no aporta diagnóstico.
    """

    QUIET_PATTERNS = (
        'GET /memory/state HTTP/',
        'GET /ahootsa/status HTTP/',
        'GET /api/v1/mic HTTP/',
        'GET /api/v1/conversation_events HTTP/',
    )

    def filter(self, record: logging.LogRecord) -> bool:  # noqa: D401
        msg = record.getMessage()
        return not any(pat in msg for pat in self.QUIET_PATTERNS)


def _configure_quiet_access_logs() -> None:
    """Filtra accesos repetitivos del panel Ahootsa.

    No cambia los logs de errores ni los eventos propios; solo elimina del
    terminal las peticiones periódicas de estado que saturan la consola.
    """
    if os.getenv("AHOOTSA_VERBOSE_HTTP_ACCESS", "0").lower() in {"1", "true", "yes", "on"}:
        return
    logger = logging.getLogger("uvicorn.access")
    if getattr(logger, "_ahootsa_quiet_filter", False):
        return
    logger.addFilter(_AhootsaAccessLogFilter())
    setattr(logger, "_ahootsa_quiet_filter", True)
...
let refreshMs=3000;
async function refresh(){{try{{const data=await j('/memory/state'); const st=data.state||data; refreshMs=st.refresh_interval_ms||3000; document.getElementById('msg').textContent=st.message||'Elige dos números.'; document.getElementById('board').innerHTML=(st.cards||[]).map(cardHtml).join('');}}catch(e){{document.getElementById('msg').textContent='No se pudo cargar el juego: '+e;}}}}
...
(async()=>{{await loadGames(); if({reset_js}) await resetGame(); else await refresh(); setInterval(()=>{{ if(!document.hidden) refresh(); }}, Math.max(1500, refreshMs||3000));}})();
</script></body></html>"""
...
button.secondary{{background:var(--pink)}} button.light{{background:#eef1f8;color:var(--navy)}} button.danger{{background:#b3261e}}
...
<div class='header'><strong>Ahootsa <b>7.0.23</b></strong><span>Conversación principal: Hugging Face Realtime oficial · Ollama auxiliar: {_ollama_model()}</span><button class='danger' onclick='exitApp()' title='Cerrar solo la app Ahootsa, no el daemon'>Salir</button></div>
...
<div class='card'><h3>COMUNICACIÓN · 3 NIVELES</h3><p class='small'>Actividades locales sin Ollama. Tres niveles: fácil, intermedio y avanzado. Actividades: saludar, pedir comida, mantener turnos y conversar.</p>
...
<input id='commActivity' value='saludar' placeholder='saludar, pedir comida, mantener turnos o conversar'/>
...
async function exitApp(){{
 if(!confirm('¿Cerrar la aplicación Ahootsa? El daemon de Reachy/MuJoCo puede seguir abierto.')) return;
 try{{ const r=await fetch('/ahootsa/exit',{{method:'POST'}}); const j=await r.json(); alert(j.message_for_user || 'Cierre solicitado.'); }}
 catch(e){{ alert('No se pudo solicitar el cierre: '+e); }}
}}
...
def mount_ahootsa_routes(app: Any, reachy_mini: Any | None = None, stop_event: Any | None = None) -> None:
    if getattr(app, "_ahootsa_7_routes_mounted", False):
        return
    setattr(app, "_ahootsa_7_routes_mounted", True)
    direct_robot = reachy_mini
    _configure_quiet_access_logs()
    ...
    @app.post("/ahootsa/exit")
    @app.get("/ahootsa/exit")
    def _exit_app() -> JSONResponse:
        """Solicita cerrar solo la app Ahootsa dentro del daemon Reachy Mini."""
        ...
        if stop_event is None:
            return JSONResponse({
                "ok": False,
                "error": "stop_event_not_available",
                "message_for_user": "No puedo cerrar la app desde este panel porque no tengo acceso al evento de parada. Usa PARAR_AHOOTSA_7_0_23.ps1.",
            }, status_code=503)
        threading.Timer(0.25, _stop_later).start()
        return JSONResponse({
            "ok": True,
            "closing": True,
            "message_for_user": "Cierre de Ahootsa solicitado. El daemon/MuJoCo puede seguir abierto; para cerrarlo todo usa PARAR_AHOOTSA_7_0_23.ps1.",
        })
    ...
    async def _communication_activities(level: str = "facil", limit: int = 4) -> JSONResponse:
        try:
    ...
    async def _communication_diagnose(level: str = "facil", activity: str = "1") -> JSONResponse:
        ...
        try:
            ...
            results["activities"] = await _call_communication_tool("list_communication_activities", "ListCommunicationActivities", level=level, limit=4)
