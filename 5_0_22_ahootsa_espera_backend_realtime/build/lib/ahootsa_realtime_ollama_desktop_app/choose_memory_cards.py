...
async def _play_reaction_now(deps: ToolDependencies, kind: str) -> dict[str, Any]:
    ...
    try:
        pe = _load_sibling_module("play_emotion_for_memory", "play_emotion.py")
        tool = pe.PlayEmotion()
        result = await tool(
            deps,
            emotion=emotion,
...
class ChooseMemoryCards(Tool):
    ...
    description = (
        ...
        "y programa una reacción emocional/sonido en segundo plano."
    )
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                message = "Ya he levantado esas cartas. Míralas con calma. Dime otros dos números."
                duplicate = {
                    "ok": True,
                    "result": "duplicate_ignored",
                    "duplicate_ignored": True,
                    "matched": False,
                    "memory_reaction_mode": "direct_response_delayed_sound_v0_4_57_4",
            ...
            intro = f"Vamos a ver la {first} y la {second}."
            reaction_kind = "invalid"

            if raw_result == "repeat_miss":
                reaction_kind = "repeat_miss"
                message = f"{intro} Esas cartas ya están visibles. Dime otros dos números."
            elif raw_result == "final" or state.get("finished"):
                reaction_kind = "final"
                message = f"{intro} ¡Felicidades! Has terminado el juego. ¿Quieres jugar otra vez o prefieres hacer otra actividad?"
            elif raw.get("matched") or raw_result in {"match", "matched"}:
                reaction_kind = "match"
                message = f"{intro} ¡Muy bien! Es pareja. Lo has hecho genial. Elige otros dos números."
            elif raw_result == "miss":
                reaction_kind = "miss"
                message = f"{intro} Casi, no son pareja. Buen intento. Prueba con otros dos números."
            elif raw.get("ok"):
                reaction_kind = "invalid"
                message = f"{intro} Mira las cartas y dime otros dos números."
            else:
                reaction_kind = "invalid"
                message = raw.get("message_for_user") or "No he podido levantar esas cartas. Dime otros dos números."

            reaction_schedule = _schedule_reaction(deps, reaction_kind)
            ...
            result = {
                ...
                "robot_next_instruction": (
                    ...
                    "Esta herramienta ha programado una única reacción en segundo plano."
                ),
            }
            _with_direct_response_fields(result, message)
            if reaction_kind == "final":
                result["suggested_next_actions"] = [
                    "Si el usuario dice 'otra vez', llama a start_memory_pairs_game con el mismo juego.",
                    "Si el usuario pide otra actividad, usa list_all_activities o la actividad solicitada."
                ]
            ...
            result = {"ok": False, "error": f"{type(exc).__name__}: {exc}", "memory_reaction_mode": "direct_response_delayed_sound_v0_4_57_4"}
            _with_direct_response_fields(result, "Ha fallado la jugada. No pasa nada. Dime otra vez dos números.")
            return result
