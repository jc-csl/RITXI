# 18 — MuJoCo y dances comunitarios v0.4.57

## Simulador MuJoCo

Sí se puede usar Ahootsa desde Reachy Mini Desktop con el daemon en modo simulación.
...
```text
1. Se abre el daemon en modo --sim.
2. MuJoCo abre una ventana 3D nativa.
3. Abres Reachy Mini Desktop.
4. Ejecutas Ahootsa.
```

## ¿Se ve en otro puerto?

La vista 3D oficial de MuJoCo normalmente se abre como ventana nativa, no como web en otro puerto.

Para verla por navegador haría falta una capa adicional:

```text
- noVNC / VNC;
- escritorio remoto;
- streaming de la ventana;
- o un visor WebGL específico.
```

Ahootsa v0.4.57 incluye el script de arranque del simulador, pero no incluye noVNC.

## Dances comunitarios

Dataset integrado por configuración:
...
play_community_dance dance="yeah_nod"
...
La librería de dances es solo movimiento. No tiene audio OGG asociado.
Las emociones sí tienen audio en la librería de emociones.
