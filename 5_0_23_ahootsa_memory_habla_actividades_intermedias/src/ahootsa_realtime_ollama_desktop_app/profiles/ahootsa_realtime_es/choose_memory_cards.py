...
async def _run_reaction_before_speech(deps: ToolDependencies, kind: str) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"ok": True, "kind": kind, "mode": "effect_then_spoken_feedback_5_0_23"}
    try:
        reaction_result = await _play_reaction_now(deps, kind)
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                message = "Ya he levantado esas cartas. Míralas con calma. Debes elegir otros dos números del 1 al 8."
                duplicate = {
                    "ok": True,
                    "result": "duplicate_ignored",
                    "duplicate_ignored": True,
                    "matched": False,
                    "memory_reaction_mode": "effect_then_spoken_feedback_5_0_23",
            ...
            if raw_result == "repeat_miss":
                ...
                message = f"{intro} Esas cartas ya están visibles. Mira con calma. Debes elegir otros dos números del 1 al 8."
            elif raw_result == "final" or state.get("finished"):
                reaction_kind = "final"
                message = f"{intro} ¡Felicidades! Has terminado el juego. ¿Quieres volver a jugar o prefieres salir?"
            elif raw.get("matched") or raw_result in {"match", "matched"}:
                reaction_kind = "match"
                message = f"{intro} ¡Muy bien! Es pareja. Lo has hecho genial. Debes elegir otros dos números del 1 al 8."
            elif raw_result == "miss":
                reaction_kind = "miss"
                message = f"{intro} Casi, no son pareja. Buen intento. Vuelve a intentarlo. Debes elegir dos números del 1 al 8."
            elif raw.get("ok"):
                reaction_kind = "invalid"
                message = f"{intro} Mira las cartas. Debes elegir dos números del 1 al 8."
            else:
                reaction_kind = "invalid"
                message = raw.get("message_for_user") or "No he podido levantar esas cartas. Debes elegir dos números del 1 al 8."
