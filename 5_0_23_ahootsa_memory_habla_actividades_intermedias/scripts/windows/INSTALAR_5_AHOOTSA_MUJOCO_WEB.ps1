...
function Copy-ProfileSafe([string]$Target) {
    ...
    "Ahootsa 5.0.23 profile copy $(Get-Date -Format o)" | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $Target ".ahootsa_5_0_6_profile_forced.txt")
...
function Test-AhootsaImportRobusto {
    ...
    $CheckPy = Join-Path $LogRoot ("ahootsa_check_import_" + $env:AHOOTSA_SESSION_ID + ".py")
    @'
import pathlib
...
Write-Step "Instalando Ahootsa 5.0.23"
