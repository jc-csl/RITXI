...
FLIP_DELAY_SECONDS = float(os.getenv("AHOOTSA_MEMORY_FLIP_DELAY_SECONDS", "1.0"))
    ...
    def _cleanup_revealed_if_needed(self) -> None:
        if self.revealed_numbers and self.reveal_until and _now() >= self.reveal_until:
            ...
            if self.last_result == "miss":
                self.message = "Las cartas se han girado. Debes elegir dos números del 1 al 8."
    ...
    def choose(self, first: int, second: int) -> dict[str, Any]:
        with self.lock:
            ...
            if is_match:
                ...
                self.message = f"¡Muy bien! {pair['left']} va con {pair['right']}. Debes elegir otros dos números del 1 al 8."
                result = "match"
            ...
            self.last_result = "miss"
            self.last_animation = "💪"
            self.message = f"Casi. {c1['label']} y {c2['label']} no son pareja. Vuelve a intentarlo. Debes elegir dos números del 1 al 8."
