...
- `app/main.py`: backend FastAPI local.
- `static/`: interfaz web, micro, TTS y botones.
- `scripts/`: reservado para utilidades.
- `test/`: scripts de diagnóstico.

## IA local

El backend llama a Ollama por `/api/chat`. No hay backend remoto.

## Actividades

Las actividades de comunicación y Memory están implementadas localmente, sin depender del LLM para continuar.
