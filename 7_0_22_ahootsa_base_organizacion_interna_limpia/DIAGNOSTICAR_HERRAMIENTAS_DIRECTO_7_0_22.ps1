...
$Out = Join-Path $LogDir "AHOOTSA_DIAGNOSTICO_DIRECTO_7_0_22_$Stamp.log"
$Py = Join-Path $env:LOCALAPPDATA "Reachy Mini Control\apps_venv\Scripts\python.exe"
if (-not (Test-Path $Py)) { $Py = "python" }
Write-Host "Ahootsa diagnostico directo 7.0.22"
Write-Host "Python: $Py"
"Ahootsa diagnostico directo 7.0.22`nPython: $Py`n" | Out-File -Encoding UTF8 -FilePath $Out
$Tmp = Join-Path $env:TEMP "ahootsa_diag_direct_7_0_22.py"
@'
...
p("\n=== COMPROBACION CODIGO PATCH 7.0.22 ===")
...
spec = importlib.util.spec_from_file_location("diag_play_emotion_7019", pe_path)
...
for name in ["list_panel_dances_activities","play_panel_dance_activity","start_memory_pairs_game","choose_memory_cards","memory_pairs_game_status","list_communication_activity_levels","list_communication_activities","start_communication_activity"]:
    try:
...
for url in [
    ...
    "http://127.0.0.1:7860/ahootsa/resolve_activity?activity=el%C3%83%C6%92%C3%82%C2%A9ctrico",
    "http://127.0.0.1:7860/ahootsa/list_activities",
    "http://127.0.0.1:7860/memory/state",
    "http://127.0.0.1:7860/memory/page?game_id=animales&reset=0",
    "http://127.0.0.1:7860/communication/levels",
    "http://127.0.0.1:7860/communication/activities?level=facil&limit=6",
    "http://127.0.0.1:7860/communication/start?level=facil&activity=1",
    "http://127.0.0.1:7860/communication/diagnose?level=facil&activity=1",
]:
    http(url)
...
$combined | Add-Content -LiteralPath $Out -Encoding UTF8
Write-Host $combined
if ($p.ExitCode -ne 0) {
