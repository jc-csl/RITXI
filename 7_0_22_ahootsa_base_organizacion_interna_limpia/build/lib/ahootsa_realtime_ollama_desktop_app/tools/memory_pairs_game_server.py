...
def _memory_timing_config() -> dict[str, float]:
    ...
    cfg = {
        ...
        "refresh_interval_ms": float(os.getenv("AHOOTSA_MEMORY_REFRESH_INTERVAL_MS", "700")),
    }
    try:
    ...
    cfg["refresh_interval_ms"] = max(250.0, min(3000.0, cfg["refresh_interval_ms"]))
    return cfg
...
def _candidate_dirs() -> list[Path]:
    # 7.0.22: datos fuera de tools/. Mantener rutas antiguas como fallback.
...
class MemoryPairsGame:
    ...
    def choose(self, first: int, second: int) -> dict[str, Any]:
        with self.lock:
            ...
            # 7.0.22: las cartas se muestran de inmediato en el iframe. En
