...
class SessionPreparationService:
    ...
    def official_profile_status(self) -> dict[str, Any]:
        ...
        url = (
            ...
            f"{config['conversation_app_port']}/personalities"
        )
    ...
    def prepare_session(
        ...
        *,
        ...
        try:
            ...
            greeting_text, greeting_uses_interests = self._build_session_greeting(
                preferred_name=preferred_name,
                interests=user.profile.interests,
                activity=activity,
                level_data=level_data,
            )
            greeting_prompt = self._build_strict_greeting_prompt(
                greeting_text
            )
            ...
            context["session"]["greeting_text"] = greeting_text
            context["session"]["greeting_uses_interests"] = (
                greeting_uses_interests
            )
            ...
            db.add_all(
                [
                    ...
                    models.SessionEvent(
                        ...
                        source="system",
                        activity=activity_key,
                        value_text=greeting_text,
                        metadata_json=json.dumps(
                            {
                                "level": level,
                                "action": "session_greeting",
                                "generated_profile": profile_name,
                                "uses_interests": greeting_uses_interests,
                            },
                            ensure_ascii=False,
                        ),
                    ),
        ...
        return {
            ...
            "greeting_uses_interests": greeting_uses_interests,
        }
    ...
    def _build_session_greeting(
        ...
        *,
        ...
        name = cls._compact_greeting_value(
            preferred_name,
            max_chars=80,
        ) or ""
        known_interests = cls._compact_greeting_value(interests)

        if known_interests:
            activity_title = cls._compact_greeting_value(
                activity.get("title") or "la actividad preparada",
                max_chars=100,
            ).lower()
            greeting = (
                f"Hola, {name}. Soy Aocha. Entre las cosas que te gustan "
                f"tengo anotado: {known_interests}. ¿Te apetece hablar de "
                f"eso o prefieres empezar con {activity_title}?"
            )
    ...
    @staticmethod
    def _build_strict_greeting_prompt(greeting_text: str) -> str:
        return f"""PRIMER TURNO OBLIGATORIO DE ESTA SESIÓN.

Di literalmente y completa la frase situada entre comillas.
No la resumas, no cambies las palabras, no omitas el nombre y no añadas nada antes.

«{greeting_text}»

Después de decirla, espera la respuesta de la persona antes de continuar.
"""
...
- El archivo greeting.txt contiene una orden literal para el primer turno.
- Di el saludo completo sin omitir el nombre preferido.
- Si el saludo menciona intereses, conserva esos intereses en la frase.
- No sustituyas el saludo por la primera pregunta de la actividad.
- Espera la respuesta de la persona antes de continuar.

CONTINUACIÓN DE LA ACTIVIDAD
Después de la respuesta al saludo, continúa con esta propuesta, adaptándola de forma natural:
