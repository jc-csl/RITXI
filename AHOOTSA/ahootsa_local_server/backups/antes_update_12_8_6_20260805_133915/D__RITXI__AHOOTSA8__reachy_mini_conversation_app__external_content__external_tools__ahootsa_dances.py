...
Version 1.3 keeps the shared-audio fix from v1.2 and avoids a Hugging Face
realtime function-result bug affecting tools that request a second model
response after execution. The dance catalogue is now supplied directly in
the profile instructions and tool description instead of through a list tool.
...
class PlayAhootsaDance(Tool):
    ...
    description = (
        "Play one dance from Ahootsa's local catalog after the user clearly "
        "chooses it. The complete available catalogue is: Baile 1=dance1; "
