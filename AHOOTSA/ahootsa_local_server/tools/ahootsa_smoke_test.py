...
def _join_wrapped_content(content: str) -> str:
    """Reconstruct text wrapped by Windows PowerShell transcripts."""
...
def read_completed_roles(log_path: Path) -> list[tuple[str, str]]:
    """Read final user/assistant turns after PowerShell closes the transcript."""
    try:
        log_text = decode_log(log_path.read_bytes())
    except FileNotFoundError as exc:
        raise SmokeTestError(
            f"No se encuentra el log de la sesión: {log_path}"
        ) from exc
    ...
    for match in entry_pattern.finditer(log_text):
        role = match.group(1).strip()
        content = _join_wrapped_content(match.group(2)).strip()
...
def extract_first_conversation_pair(
    log_path: Path,
) -> tuple[str, str, list[tuple[str, str]]]:
    roles = read_completed_roles(log_path)

    for index, (role, content) in enumerate(roles):
        if role != "user":
            continue

        for next_role, next_content in roles[index + 1 :]:
            if next_role == "user":
                break
            if next_role == "assistant":
                return content, next_content, roles

    raise SmokeTestError(
        "El log final no contiene un turno completo de usuario seguido "
        "de una respuesta de Aocha."
    )


def wait_for_manual_conversation() -> None:
    ...
    print("   Espera a que Aocha responda.")
    print("")
    input(
        "   Cuando Aocha haya respondido, pulsa INTRO en esta consola: "
    )
...
def main() -> int:
    ...
    try:
        print("=" * 70)
        print("PRUEBA FINAL AHOOTSA 12.5.2")
        ...
        report_path = session_directory / "smoke_test_report_12_5_2.json"
        ...
        wait_for_manual_conversation()

        service_ports_during_conversation = {
            "daemon_8000": port_open(8000),
            "conversation_app_7860": port_open(7860),
            "local_server_8100": port_open(8100),
        }
        if not all(service_ports_during_conversation.values()):
            raise SmokeTestError(
                "Algún servicio dejó de estar activo durante la conversación: "
                f"{service_ports_during_conversation}"
            )

        print_step(9, "Cerrar la Conversation App y finalizar el log")
        stop_conversation_app(project_root)

        deadline_for_log = time.monotonic() + 15
        while time.monotonic() < deadline_for_log:
            if log_path.exists() and log_path.stat().st_size > 0:
                time.sleep(1)
                break
            time.sleep(0.5)

        user_text, assistant_text, parsed_roles = (
            extract_first_conversation_pair(log_path)
        )
        print(f"   Usuario recuperado: {user_text}")
        print(f"   Aocha recuperada: {assistant_text}")
        print(f"   Turnos finales encontrados en el log: {len(parsed_roles)}")

        print_step(10, "Registrar las transcripciones en la sesión")
        ...
        print_step(11, "Finalizar la sesión desde el panel profesional")
        finished = finish_session(
            "Prueba final 12.5.2 completada con conversación real "
        ...
        print_step(12, "Verificar API, SQLite y resumen")
        ...
        report = {
            "test": "AHOOTSA_SMOKE_TEST_12_5_2",
            "success": True,
            "verified_at": now_iso(),
            ...
            "services": {
                "before_conversation": service_ports,
                "during_conversation": service_ports_during_conversation,
            },
            "conversation": {
                "user": user_text,
        ...
        print("\n" + "=" * 70)
        print("PRUEBA FINAL 12.5.2 VALIDADA")
        ...
        if prepared is not None and not session_finished:
            try:
                finish_session(
                    "Prueba final 12.5.2 abortada: " + message,
                    decision="no_decision",
                )
                session_finished = True
            except Exception:
