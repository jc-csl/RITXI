...
def parse_final_roles(log_path: Path) -> list[dict[str, Any]]:
    ...
    for source_index, match in enumerate(entry_pattern.finditer(log_text)):
        ...
        content = join_wrapped_content(match.group(3)).strip()
        ...
        if (
            ...
            continue
        if (
            "tool play_ahootsa_dance" in lowered
            and "args" in lowered
        ):
            continue
...
def transcript_end_time(log_path: Path) -> datetime | None:
    if not log_path.is_file():
        return None
...
def terminal_listening_diagnostic(
    *,
    log_path: Path,
    ...
    result["last_role"] = last_turn.get("role")
    result["last_turn_at"] = last_turn.get("timestamp")

    end_time = transcript_end_time(log_path)
    if end_time is None:
        return result
    ...
    if (
        ...
        minutes = silence_seconds / 60.0
        result["observation"] = (
            "Posible incidencia de escucha: después del último "
            "turno de Aocha no se registró ninguna intervención "
            f"de la persona durante {minutes:.1f} minutos antes "
            "del cierre. Puede corresponder a silencio voluntario "
            "o a una interrupción del micrófono/VAD."
        )
    ...
    for item in dialogue:
        ...
        created = http_json(
            (
                ...
                f"{session_id}/conversation-events"
            ),
            method="POST",
            body=payload,
        )
...
def make_report(
    *,
    ...
    audio_diagnostics = terminal_listening_diagnostic(
        log_path=log_path,
        dialogue=dialogue,
    )
    observations = list(
        summary.get("automatic_observations", [])
    )
    listening_observation = audio_diagnostics.get(
        "observation"
    )

    if (
        isinstance(listening_observation, str)
        and listening_observation
        and listening_observation not in observations
    ):
        observations.append(listening_observation)
