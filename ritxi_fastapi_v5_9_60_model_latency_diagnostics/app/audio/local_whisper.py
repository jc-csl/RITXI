...
Módulo de STT local basado en faster-whisper.

Responsabilidades:
- recibir audio WAV desde el navegador;
- cargar el modelo Whisper local;
- transcribir voz a texto;
- aplicar correcciones de vocabulario si la actividad es cerrada;
- descartar repeticiones/alucinaciones típicas del STT;
- NO aplicar vocabularios cerrados cuando la actividad es abierta (`open_text`, `open_name`).

Este módulo debe ser tolerante: un error en filtros de vocabulario no debe romper
