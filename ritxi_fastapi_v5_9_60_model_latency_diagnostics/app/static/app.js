...
app.js es el controlador principal del panel web de Ritxi.

Responsabilidades:
- gestionar la interfaz de chat;
- activar/parar micrófono;
- enviar audio a STT local;
- enviar texto a Ollama mediante /api/chat;
- reproducir voz con TTS del navegador;
- lanzar movimientos/emociones en Reachy;
- pintar tarjetas de emociones, actividades, juegos y tutor/apoyo;
- gestionar ciclos con turnos;
- editar configuración desde el panel.

Flujo principal:
usuario habla/escribe -> app.js -> FastAPI /api/... -> servicios Python -> app.js actualiza UI.
*/
...
const SHORT_ACTION_IDS = [
  ...
  'palmas_lentas','palmas_rapidas','baile_suave','baile_divertido','imitame','chiste','fiesta'
];
...
const ACTION_GROUPS = [
  { title: 'Emociones oficiales Pollen Robotics', target: 'emotionActionGrid', items: [
    { id:'wake_up', title:'Despertar', icon:'☀️', emotion:'wake_up', text:'', subtitle:'Reset postura + audio oficial', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'cheerful1', title:'Alegría / cariño', icon:'🥰', emotion:'cheerful1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true },
    { id:'enthusiastic1', title:'Entusiasmo', icon:'🤩', emotion:'enthusiastic1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'gasp1', title:'Sorpresa', icon:'😲', emotion:'gasp1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'amazed1', title:'Asombro', icon:'🌟', emotion:'amazed1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'curious1', title:'Curiosidad', icon:'🔎', emotion:'curious1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'attentive1', title:'Atento', icon:'👂', emotion:'attentive1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'calming1', title:'Calma', icon:'🍃', emotion:'calming1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'boredom1', title:'Aburrido / pensar', icon:'🤔', emotion:'boredom1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'confused1', title:'Confundido', icon:'😕', emotion:'confused1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'yes1', title:'Sí / OK', icon:'👍', emotion:'yes1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'no1', title:'No / rechazo', icon:'👎', emotion:'no1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'sad1', title:'Triste', icon:'😭', emotion:'sad1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'anxiety1', title:'Ansiedad / asco', icon:'🤢', emotion:'anxiety1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'angry1', title:'Enojo', icon:'😡', emotion:'angry1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'sleep1', title:'Sueño', icon:'😴', emotion:'sleep1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
  ]},
  { title: 'Bailes y expresividad', target: 'emotionActionGrid', items: [
    { id:'dance1', title:'Baile oficial 1', icon:'🕺', emotion:'dance1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'dance2', title:'Baile oficial 2', icon:'💃', emotion:'dance2', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'dance3', title:'Baile oficial 3', icon:'🤖', emotion:'dance3', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    ...
    { id:'fiesta', title:'Fiesta', icon:'🪩', emotion:'celebracion', text:'¡Fiesta! Celebramos el esfuerzo y las ganas de participar.', subtitle:'Celebración + voz' },
    { id:'electric1', title:'Energía eléctrica', icon:'⚡', emotion:'electric1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
  ...
  { title: 'Gestos sociales y comunicación', target: 'emotionActionGrid', items: [
    { id:'hello1', title:'Saludo / hola', icon:'🙋', emotion:'hello1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    ...
    { id:'come1', title:'Ven conmigo', icon:'🤝', emotion:'come1', text:'Ven conmigo. Vamos paso a paso.', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'pulgar_arriba', title:'Pulgar arriba', icon:'👍', emotion:'yes1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'no_suave', title:'No suave', icon:'👎', emotion:'no1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
    { id:'corazon', title:'Corazón', icon:'❤️', emotion:'cheerful1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
  ...
  { title: 'Tutor / Terapia y apoyo emocional', target: 'emotionActionGrid', items: [
    ...
    { id:'relajate', title:'Relájate', icon:'🍃', emotion:'calming1', text:'', subtitle:'Movimiento oficial + audio', recordedAudio:true, fallbackText:'Ritxi ejecuta esta emoción.' },
...
async function api(path, options = {}) {
  ...
  const res = await fetch(path, { headers: isForm ? {} : { 'Content-Type': 'application/json' }, ...options });
...
function renderLogs() {
  ...
  const rows = logEvents.filter(e => (logFilter==='all' || e.level===logFilter || (logFilter==='warning' && e.level==='warn')) && (!search || `${e.origin} ${e.message}`.toLowerCase().includes(search))).slice(0,80);
  tbody.innerHTML = rows.map(e => `<tr><td>${e.time}</td><td><span class="level ${e.level==='warn'?'warning':e.level}">${(e.level==='warn'?'WARN':e.level).toUpperCase()}</span></td><td>${e.origin}</td><td>${escapeHtml(e.message)}</td><td>…</td></tr>`).join('');
  $('logCount').textContent = `${rows.length} / ${logEvents.length} eventos`;
}
...
function modelSpeedInfo(model){
  ...
  if(m.includes('qwen3:0.6b')) return {label:'Muy rápido', level:'fast', note:'modelo pequeño'};
  if(m.includes('gemma3:1b')) return {label:'Rápido recomendado', level:'fast', note:'equilibrado'};
...
function updateModelPills(model=null){
  ...
  if($('modelSpeedPill')){
    $('modelSpeedPill').textContent = speed.label;
...
function initChat(){ addChat('bot','¡Hola! Soy Ritxi 🤖 ¿Cómo estás hoy?'); addChat('user','Hola Ritxi, ¿qué puedes hacer?'); addChat('bot','Puedo conversar contigo, contar chistes, bailar, mostrar emociones, ayudarte a relajarte y enseñarte muchas cosas.\n\nSolo dime lo que te apetezca y lo haremos juntos. 😊'); }
...
async function refreshStatus(){
  try{
    ...
    $('currentState').textContent = status.turn_state === 'idle' ? 'Listo para conversar' : status.turn_state;
    $('currentSubState').textContent = canListenNow(status) ? 'Esperando tu mensaje' : 'Procesando / protegido';
    $('latencyPill').textContent = status.last_latencies?.llm_total_ms ? `${Math.round(status.last_latencies.llm_total_ms)} ms` : '— ms';
    ...
    $('statusCompact').textContent = JSON.stringify({robot:status.robot,llm:status.llm,stt:status.stt,tts:status.tts,cola:status.scheduler,echo_guard:status.echo_guard,warnings:status.warnings},null,2);
    $('logStatus').textContent = `Log: ${status.logs.session_log_file}`;
...
async function setRealtime(enabled){ 
  if(enabled){
    ...
    logClient('info','mic','Conversación/micro activada de forma directa',{flags:flags(),mic_mode:selectedMicMode()});
    return;
  }
...
async function playOfficialRecordedAudio(emotionId, title='emoción'){
  if(!flags().output_audio){ logClient('debug','audio',`Audio oficial omitido por Audio salida desactivado: ${emotionId}`); return {started:false, skipped:'output_audio_disabled'}; }
  try{
    stopEffectAudio();
    pauseAlwaysOnMic('Audio oficial HF');
    ...
    await setClientSpeaking(true,'official-wav-start');
    setMicStatus(`Reproduciendo sonido oficial: ${title}`);
    logClient('info','audio',`Reproduciendo WAV oficial por navegador: ${emotionId}`,{url});
...
async function speakWithBrowser(text){ 
  ...
  const shouldListenAfter = realtimeEnabled || activityAwaitingUser || autoListenAfterBot || activityAutoListenAfterBot || micAlwaysOn;

  if(!clean || !flags().output_audio || !flags().tts || !flags().auto_voice_response){
  ...
  pauseAlwaysOnMic('Ritxi hablando');
  stopBrowserTts(); 
  await setClientSpeaking(true,'browser-tts-start'); 
  setMicStatus('Ritxi está hablando. Micro pausado para evitar eco…'); 
  logClient('info','tts','Reproduciendo audio con voz del navegador',{len:clean.length}); 
  ...
  await setClientSpeaking(false,'browser-tts-end'); 
  ...
  if(shouldListenAfter && !manualMicStopLock){
    ...
    setMicStatus('Ritxi terminó de hablar. Micro parado.');
  }
}
...
async function sendTurn({source='manual'}={}){
  ...
  const activeContextForTurn = currentActivityContext || (activityAwaitingUser || activityAutoListenAfterBot || shortCycleWaitingForNext ? lastActivityContext : null);
  ...
  if(manualLike){
    ...
    // Paramos micro/STT, pero conservamos activeContextForTurn para contestar dentro de la actividad.
  ...
  const sentText = text;
  $('message').value='';
  logClient('info','chat',`Usuario: ${sentText}`,{source,wasActivityTurn,flags:flags(),model:selectedModel,activity_context:activeContextForTurn});
  logClient('debug','chat','Preparando envío de texto a actividad/IA',{source,hasContext:!!activeContextForTurn});
  logClient('info','ollama',`Modelo real usado en turno: ${selectedModel}`,modelSpeedInfo(selectedModel));
  ...
  try{ 
    ...
    const textForLLM = enrichUserTextWithActivityContext(sentText, activeContextForTurn);
    const payload={session_id:$('sessionId').value||'demo',text:textForLLM,flags:flags(),llm_model:selectedModel}; 
    const data=await api('/api/chat',{method:'POST',body:JSON.stringify(payload),signal:controller.signal}); 
    if(watchdog){ clearTimeout(watchdog); watchdog=null; }
    updateModelPills(data.model || selectedModel);
    if(flags().output_text) addChat('bot',data.text,`emoción: ${data.emotion} · modelo: ${data.model || selectedModel}`); 
    rememberLatency(data.latencies); 
    logClient('info','ollama',`Respuesta recibida (${Math.round(data.latencies?.llm_total_ms || 0)} ms)`,{provider:data.provider,model:data.model || selectedModel,warnings:data.warnings}); 
    if(flags().robot_motion && flags().move_talk_generate_text) setExecuting(`Respuesta: ${data.emotion}`,'Texto + movimiento + audio'); 
    await refreshStatus(); 
    if(flags().tts){ speakWithBrowser(data.text).catch(err=>logClient('warn','tts',`TTS no bloqueante con aviso: ${String(err)}`)); }
    refreshStatus().catch(()=>{}); 
    lastBotTurnDoneTs = Date.now();
    if(activeContextForTurn && !shortCycleActive){
      if(isMaxCreativeActivityContext(activeContextForTurn) && !userAskedToEndActivity(sentText)){
        currentActivityContext = activeContextForTurn;
        lastActivityContext = activeContextForTurn;
        activityAwaitingUser = true;
        activityAutoListenAfterBot = true;
        $('message').placeholder = 'Seguimos la actividad larga: habla o escribe aquí...';
        setTimeout(()=>scheduleMicOpenAfterSpeech({autoSend:true, reason:'maxima-interaccion-continua'}), 350);
      }else{
    ...
    if(!manualMicStopLock && (wasActivityTurn || activityAutoListenAfterBot)){
      activityAwaitingUser=true;
      activityAutoListenAfterBot=true;
...
function updateInteractionModeState(name='full'){
  ...
  const hint=$('interactionModeHint');
  if(hint){
    if(name==='text') hint.textContent='Solo texto: micro apagado; la IA local responde por escrito.';
    else if(name==='voiceRobot') hint.textContent='Micro + IA: voz activa, texto sigue disponible, robot parado.';
    else hint.textContent='Completo: texto + micro + IA + voz + robot.';
  }
}

function markPresetButton(name){
  const map={text:'presetTextOnlyBtn',voiceRobot:'presetVoiceRobotBtn',full:'presetFullTutorBtn'};
  ['presetTextOnlyBtn','presetVoiceRobotBtn','presetFullTutorBtn','textModeBtn'].forEach(id=>$(id)?.classList.remove('active-preset'));
...
function setModulePreset(name){
  // Texto: apaga micro/STT y prioriza entrada escrita.
  // Micro: activa voz/STT/LLM/TTS pero mantiene la caja de texto como respaldo.
  // Tutor completo: activa todo sin desactivar texto.
  const sets={
    text:{
      inputMicrophone:false,sttEnabled:false,llmEnabled:true,ttsEnabled:false,outputAudio:false,outputText:true,
  ...
  const cfg=sets[name]||sets.full;
  Object.entries(cfg).forEach(([id,val])=>{ if($(id)) $(id).checked=!!val; });
  markPresetButton(name);
  enableTextInputAlways(`preset-${name}`);
  logClient('info','config',`Modo de interacción aplicado: ${name}`,cfg);
  if($('activeWait')) toggleIdleFromCheckbox().catch(()=>{});

  if(name==='text'){
    enterTextPriorityMode('preset-texto',{deepStop:true});
    setMicStatus('Modo Texto: micro apagado. Escribe y pulsa ➤.');
  }else if(name==='voiceRobot'){
    manualMicStopLock=false;
    realtimeEnabled=true;
    autoListenAfterBot=true;
    activityAutoListenAfterBot=false;
    setMicStatus('Modo Micro: habla cuando quieras. El texto sigue disponible.');
    updateMicToggleUi();
    ...
    realtimeEnabled=true;
    autoListenAfterBot=true;
    setMicStatus('Tutor completo: texto, micro, IA, voz y robot activos.');
    updateMicToggleUi();
...
const GAME_TAB_ACTION_IDS = new Set([
  'dance1','dance2','dance3','baile_divertido','fiesta','electric1','baile_suave','ritmo_palmas',
...
const GAME_ACTION_GROUPS = [
  { title:'Bailes y movimiento divertido', target:'gamesActionGrid', items:[
    ...
    { id:'fiesta', title:'Fiesta', icon:'🎉', emotion:'celebracion', text:'¡Fiesta! Celebramos que estamos practicando juntos.', subtitle:'Celebración + voz' },
...
function isTurnShortActivity(item){
  ...
  return !!item.awaitUser || ['saludo_corto','si_corto','no_corto','turno_corto','ayuda_corta','despedida_corta','animal_corto','palabra_corta','calma_corta','preguntar_nombre','pedir_turno','pedir_ayuda','buscar_palabra','sinonimos','opuestos','completar_frase','explicar_imagen','frase_corta','historia_turnos'].includes(item.id);
}
...
function fastActivityReply(text, context){
  ...
  if(isMaxCreativeActivityContext(context)){
    if(userAskedToEndActivity(text)){
...
async function promptUserAfterActivity(item,{autoListen=true,autoSend=true}={}){
  ...
  if(autoListen && flags().input_microphone && flags().stt){
    ...
    setMicStatus('Tu turno: micro/STT desactivado. Escribe en el chat y pulsa ➤.');
  }
}
...
async function runActivity(item,btn=null,options={}){
  ...
  try{
    ...
    logClient('info','activity',`Actividad iniciada: ${item.title}`,item);
    if(flags().output_text) addChat('bot',`▶️ ${item.title}: ${item.intro || 'vamos a empezar.'}`,'actividad');
    ...
    for(let i=0;i<steps.length;i++){
      ...
      if(step.chat && flags().output_text) addChat('bot',step.chat,`actividad: ${label}`);
      ...
      if(flags().robot_motion){
        await api('/api/robot/action',{method:'POST',body:JSON.stringify({emotion,text:null,motion_enabled:true,audio_enabled:false,return_to_neutral:false,layer:'manual',wait:false,speech_motion:flags().speech_motion})}).catch(err=>logClient('warn','robot',`Movimiento de actividad con aviso: ${String(err)}`));
      }
      if(step.text) await speakWithBrowser(step.text);
      ...
      if(step.recordedAudio) await playOfficialRecordedAudio(step.emotion || item.emotion, step.label || item.title);
      if(step.pauseAfter) await sleep(step.pauseAfter);
    }
    if(flags().robot_motion){ await api('/api/robot/action',{method:'POST',body:JSON.stringify({emotion:'neutral',text:null,motion_enabled:true,audio_enabled:false,return_to_neutral:false,layer:'manual',wait:false,speech_motion:false})}).catch(()=>{}); }
    setExecuting(item.title,'Actividad finalizada');
    if((isTurnShortActivity(item) || item.awaitUser || item.requiresInput || activityNeedsMicroPrompt(item)) && !options.fromCycle){
...
async function executeAction(item,btn=null,options={}){
  ...
  try{
    ...
    logClient('info','robot',`Acción lanzada: ${item.title}`,item);
    const hasText = !!(item.text && item.text.trim());
    if(flags().move_talk_generate_text && hasText) addChat('bot',item.text,`acción: ${item.title}`);
    const useRecordedAudio = !!item.recordedAudio;
    const fallbackText = (item.fallbackText || item.text || `Ritxi ejecuta ${item.title}.`).trim();

    let officialAudioPromise = Promise.resolve({started:false, skipped:'not-recorded'});
    if(useRecordedAudio){
      officialAudioPromise = playOfficialRecordedAudio(item.emotion, item.title);
    }

    await waitForRobotMotionSlot(item?.id || 'executeAction');

    const motionResult = flags().robot_motion
      ? await api('/api/robot/action',{method:'POST',body:JSON.stringify({
          emotion:item.emotion,
          text: useRecordedAudio ? null : (hasText ? item.text : null),
          motion_enabled:true,
          audio_enabled:!useRecordedAudio && hasText,
          return_to_neutral:true,
          layer:'manual',
          wait:useRecordedAudio,
          speech_motion:flags().speech_motion
        })}).catch(err=>({error:String(err)}))
      : {skipped:'robot_motion_disabled'};
    ...
    if(useRecordedAudio){
      if(!audioResult?.started && flags().output_audio){
        logClient('error','audio',`No se pudo reproducir el WAV oficial; revisa Hugging Face/cache para ${item.emotion}`,audioResult);
        if(flags().tts) await speakWithBrowser(fallbackText);
      }
    } else if(hasText){
...
function interactionLevelForActivity(item){
  ...
  if(LONG_INTERACTION_ACTIVITY_IDS.has(id)){
...
async function toggleIdleFromCheckbox(){ 
  ...
  if(enabled && flags().robot_motion){
    ...
    await api('/api/robot/action',{method:'POST',body:JSON.stringify({emotion:'neutral',text:null,motion_enabled:true,audio_enabled:false,return_to_neutral:false,layer:'manual',wait:false,speech_motion:false})}).catch(()=>{});
  }
  await refreshStatus(); 
}
...
function exportLogs(){ const blob=new Blob([logEvents.map(e=>JSON.stringify(e)).join('\n')],{type:'application/jsonl'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`ritxi_v5_logs_${Date.now()}.jsonl`; a.click(); URL.revokeObjectURL(a.href); }
...
async function loadInlineConfigFile(path='system_prompt'){
  ...
  try{
    ...
    if(real) real.textContent = detail.path || path;
    if(status) status.textContent = `Cargado: ${inlineConfigSelectedLabel}. Puedes editar y guardar.`;
    logClient('info','config',`Archivo de configuración cargado: ${path}`);
  }catch(err){
    editor.value = `Error cargando ${path}:\n${String(err)}`;
    if(status) status.textContent = 'Error cargando archivo.';
...
function initInlineConfigEditor(){
  document.querySelectorAll('.config-quick-file').forEach(btn=>{
    btn.addEventListener('click',()=>loadInlineConfigFile(btn.dataset.configPath || 'system_prompt'));
  });
...
$('modelSelect')?.addEventListener('change',()=>{updateModelPills(); logClient('info','ollama',`Modelo seleccionado: ${selectedModelName()}`);}); $('modelSelect')?.addEventListener('change',()=>logClient('info','ollama',`Modelo seleccionado: ${$('modelSelect').value}`,{modelSelectChange:true}));
  $('resetBtn')?.addEventListener('click',async()=>{forceUnlockTurnState('reset'); manualMicStopLock=false; activityAutoListenAfterBot=false; activityAwaitingUser=false; await api(`/api/sessions/${$('sessionId').value||'demo'}/reset`,{method:'POST',body:'{}'}); $('chatHistory').innerHTML=''; initChat(); updateMicToggleUi(); logClient('info','chat','Sesión reiniciada');});
  ...
  $('testAudioBtn')?.addEventListener('click',()=>speakWithBrowser('Prueba de audio de Ritxi. Si me escuchas, el altavoz funciona correctamente.'));
  ...
  $('pauseLogsBtn').addEventListener('click',()=>{logPaused=!logPaused;$('pauseLogsBtn').textContent=logPaused?'▶ Reanudar':'Ⅱ Pausar'; if(!logPaused)renderLogs();}); $('clearLogsBtn').addEventListener('click',()=>{logEvents=[];renderLogs();}); $('exportLogsBtn').addEventListener('click',exportLogs); $('saveLogBtn')?.addEventListener('click',exportLogs); $('saveLogBtn2').addEventListener('click',exportLogs);
  $('logSearch').addEventListener('input',renderLogs); document.querySelectorAll('[data-log-filter]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-log-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');logFilter=b.dataset.logFilter;renderLogs();}));
...
function boot(){
  ...
  bindEvents();
  initChat();
  logClient('info','app','Ritxi v5.9.44 iniciado: fichas restauradas y micro manual corregido');
  ...
  updateInteractionModeState('full');
