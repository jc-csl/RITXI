...
impl PeripheralInternal {
    ...
    pub fn set_characteristics(
        ...
        // Merge new characteristics into existing map instead of replacing,
        // to preserve descriptor state from earlier discovery callbacks.
        // See: https://github.com/deviceplug/btleplug/issues/397
        for (characteristic_uuid, characteristic) in characteristics {
            if !service.characteristics.contains_key(&characteristic_uuid) {
    ...
    pub fn set_characteristic_descriptors(
        ...
        let Some(service) = self.services.get_mut(&service_uuid) else {
            warn!(
                "Got descriptors for unknown service {}, ignoring",
                service_uuid
            );
            return;
        };
        let Some(characteristic) = service.characteristics.get_mut(&characteristic_uuid) else {
            warn!(
                "Got descriptors for unknown characteristic {} in service {}, ignoring",
