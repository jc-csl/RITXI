...
class MockLLMProvider:
    ...
    def _response_for(self, messages: list[dict[str, str]]) -> str:
        ...
        if "vida" in lowered:
            return "[CURIOSO] La vida tiene muchas cosas importantes: aprender, cuidar a otros, descansar y disfrutar poco a poco."
        if "perro" in lowered and "gato" in lowered:
            return "[CURIOSO] Un perro suele ser muy sociable y le gusta jugar. Un gato suele ser más tranquilo e independiente. Los dos pueden ser buenos compañeros."
        if "perro" in lowered:
            return "[CURIOSO] Un perro es un animal sociable. Puede jugar, aprender órdenes sencillas y acompañar a las personas."
        if "gato" in lowered:
            return "[CURIOSO] Un gato es un animal tranquilo y curioso. Le gusta explorar y descansar en lugares cómodos."
        ...
        return "[ESCUCHA_ACTIVA] Entendido. Te respondo sobre el tema que me digas, con una frase corta."
