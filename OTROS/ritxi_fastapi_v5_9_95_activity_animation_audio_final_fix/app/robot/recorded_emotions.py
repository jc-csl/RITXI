...
class RecordedEmotionLibrary:
    ...
    def get_recorded_audio_path(self, emotion_id: str) -> tuple[str | None, str | None, str | None]:
        """Devuelve/descarga el audio oficial de Hugging Face para una emoción.

        El dataset oficial cambió de WAV a OGG en versiones recientes. Por eso
        se prueban ambas extensiones, priorizando OGG. Devuelve
        (executed_id, audio_path, error).
        """
        self._ensure_loaded()
        ...
        # 1) Intentar desde el objeto RecordedMoves: puede tener audio_path local cacheado.
        ...
        # 2) Descargar directamente desde HF. El dataset actual usa .ogg;
        #    se deja .wav como compatibilidad con versiones antiguas/caché local.
        try:
                ...
                try:
                    ...
                    if os.path.exists(path):
                        log_event(
                            ...
                            requested=emotion_id,
                            executed_id=candidate,
                            audio_path=path,
                            extension=ext,
                        )
        ...
        error = "; ".join(errors[-6:]) or "audio oficial no encontrado"
