...
async function robotStatus() {
  try {
    ...
    robotText.textContent = String(e);
  }
}

async function robotProbe() {
  try {
    robotText.textContent = "Probando SDK...";
    const r = await fetch("/api/robot/probe", { method: "POST" });
    const data = await r.json();
    robotText.textContent = JSON.stringify(data, null, 2);
  } catch (e) {
    robotText.textContent = String(e);
...
async function robotAction(action, silent = false) {
  try {
    if (!silent) robotText.textContent = `Ejecutando ${action}...`;
    const r = await fetch(`/api/robot/action/${action}`, { method: "POST" });
    const data = await r.json();
    robotText.textContent = JSON.stringify(data, null, 2);
  } catch (e) {
    robotText.textContent = String(e);
...
addMessage("Hola. Soy Ahootsa en modo local. Pulsa Hablar o escribe. Si el reconocimiento falla, usa Win + H sobre la caja de texto.", "bot", "sistema");
