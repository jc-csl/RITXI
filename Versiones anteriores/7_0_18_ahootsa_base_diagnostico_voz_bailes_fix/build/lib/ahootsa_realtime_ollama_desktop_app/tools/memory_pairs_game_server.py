...
def _memory_timing_config() -> dict[str, float]:
    ...
    tools/memory_timing_config.json. Se lee de forma dinámica para que cambiar
    ...
    try:
        path = Path(__file__).resolve().with_name("memory_timing_config.json")
        if path.exists():
...
class MemoryPairsGame:
    ...
    def choose(self, first: int, second: int) -> dict[str, Any]:
        with self.lock:
            ...
            # 7.0.18: las cartas se muestran de inmediato en el iframe. En
