...
def event_log_path() -> Path:
    ...
    return log_root() / f"ahootsa5_{session_id()}_eventos.jsonl"
...
def runtime_log_path() -> Path:
    ...
    if explicit:
        return Path(explicit)
    return log_root() / f"ahootsa5_{session_id()}_runtime.log"
...
def screen_log_path() -> Path:
    ...
    if explicit:
        return Path(explicit)
    return log_root() / f"ahootsa5_{session_id()}_pantalla.log"
