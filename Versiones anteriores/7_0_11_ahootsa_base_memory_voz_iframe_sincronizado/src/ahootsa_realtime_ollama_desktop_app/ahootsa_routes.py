...
"""Ahootsa 7.0.11 panel/API routes.
...
function cardHtml(c){{const open=c.visible||c.matched; const isSel=selected.includes(c.number); const cls=(open?'card open'+(c.matched?' matched':''):'card'+(isSel?' selected':'')); return `<button class="${{cls}}" onclick="pick(${{c.number}})" ${{c.matched?'disabled':''}}>${{open?`<div class="ico">${{c.icon||''}}</div><div>${{c.label||''}}</div><div class="kind">${{c.kind_label||''}}</div>`:c.number}}</button>`;}}
...
(async()=>{{await loadGames(); if({reset_js}) await resetGame(); else await refresh(); setInterval(refresh,900);}})();
</script></body></html>"""
...
<div class='header'><strong>Ahootsa <b>7.0.11</b></strong><span>Conversación principal: Hugging Face Realtime oficial · Ollama auxiliar: {_ollama_model()}</span></div>
