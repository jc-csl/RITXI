# Corrección Ahootsa 5.0.32 — `param(...)` del lanzador PowerShell
...
La 5.0.32 aplica un parche conservador:

1. Busca `LANZAR_5_AHOOTSA_MUJOCO_WEB.ps1` en la carpeta 5.0.25.
2. Elimina bloques inyectados de 5.0.31/5.0.32 si aparecen antes de `param(...)`.
...
5. El wrapper `LANZAR_5_0_32_AHOOTSA_MUJOCO_WEB.ps1` crea un timestamp nuevo en `$env:AHOOTSA_SESSION` antes de llamar al lanzador original.
