"""
Ahootsa 5.0.32 - reparacion del lanzador PowerShell + logs por ejecucion.
...
La reparacion:
- elimina bloques inyectados 5.0.31/5.0.32 que puedan estar antes de param(...);
...
MARKER = "Ahootsa 5.0.32 - reparacion launcher param"
...
# ============================================================
# Ahootsa 5.0.32 - espera backend segura
...
def backup(path: pathlib.Path, original: str) -> pathlib.Path:
    ...
    b = path.with_suffix(path.suffix + f".bak_5_0_32_{ts}")
...
def remove_ahootsa_injected_blocks(text: str) -> tuple[str, int]:
    ...
    original = text
    patterns = [
        # Bloque completo con cabeceras de separadores de 5.0.31/5.0.32.
...
def patch_wait_script(path: pathlib.Path) -> tuple[bool, str]:
    ...
    if "Ahootsa 5.0.32 - espera backend segura" in original:
        return False, "ESPERAR ya estaba en formato 5.0.32"
    b = backup(path, original) if path.exists() else None
    write_text(path, WAIT_SCRIPT)
    return True, f"ESPERAR reemplazado por version canonica 5.0.32; backup={b.name if b else 'no_existia'}"
...
def main() -> int:
    ...
    print("============================================================")
    print("Ahootsa 5.0.32 - reparacion launcher param + logs limpios")
    print("============================================================")
    print("Target:", root)
    ...
    print("\n[OK] Correccion 5.0.32 de launcher/logs terminada.")
    return 0
