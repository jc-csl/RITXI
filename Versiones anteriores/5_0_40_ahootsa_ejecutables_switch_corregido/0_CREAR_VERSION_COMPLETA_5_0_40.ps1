...
Write-Host "============================================================"
Write-Host "Ahootsa 5.0.40 - crear version completa estable"
Write-Host "============================================================"
...
if ($HereFull -ieq $TargetFull) {
    throw "No ejecutes el creador usando como TargetRoot la misma carpeta del ZIP. Usa el destino por defecto D:\RITXI\5_0_40_ahootsa_estable_llm_camara o cambia -TargetRoot."
}
...
Copy-Item -LiteralPath (Join-Path $Here "tools\patch_5_0_40_modelo_audio.py") -Destination (Join-Path $TargetRoot "tools\patch_5_0_40_modelo_audio.py") -Force
Copy-Item -LiteralPath (Join-Path $Here "README.md") -Destination (Join-Path $TargetRoot "README_5_0_40.md") -Force
...
Write-Host "[INFO] Aplicando parche modelo/audio 5.0.40 en apps_venv..."
& $Py (Join-Path $TargetRoot "tools\patch_5_0_40_modelo_audio.py") --provider $Provider --ollama-model $OllamaModel --hf-model-path $HFModelPath
if ($LASTEXITCODE -ne 0) { throw "No se pudo aplicar el parche 5.0.40." }
...
'@
Set-Content -LiteralPath (Join-Path $TargetRoot "LANZAR_AHOOTSA_5_0_40.ps1") -Value $launcher -Encoding UTF8
...
Write-Host "         powershell -ExecutionPolicy Bypass -File .\LANZAR_AHOOTSA_5_0_40.ps1"
