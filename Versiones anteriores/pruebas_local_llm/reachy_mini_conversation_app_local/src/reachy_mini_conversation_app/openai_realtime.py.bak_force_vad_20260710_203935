...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    async def receive(self, frame: Tuple[int, NDArray[np.int16]]) -> None:
        ...
        # FullASR -> LLM -> TTS)
            ...
            # Skip sending to OpenAI in full local local mode: use built-in VAD + ASR + LLM + TTS
        if self._is_full_local_mode:
            # Process with built-in VAD
            speech_started, speech_ended = self._local_vad.process(audio_frame)

            # Optional forced end for Windows/FastRTC cases where silence is not detected.
            try:
                force_end_seconds = float(__import__("os").getenv("VAD_FORCE_END_SECONDS", "0") or "0")
            except Exception:
            ...
            if speech_started and not self._is_speech_active:
                ...
                # Avoid restarting/clearing the buffer on repeated "speech started" events.
