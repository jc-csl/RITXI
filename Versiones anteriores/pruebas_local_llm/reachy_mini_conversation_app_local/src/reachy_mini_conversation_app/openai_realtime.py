...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    def __init__(self, deps: ToolDependencies, gradio_mode: bool = False, instance_path: Optional[str] = None):
        ...
        self._vad_force_start_time: float | None = None  # Forced turn-end timer for noisy WebRTC/VAD environments
    ...
    async def receive(self, frame: Tuple[int, NDArray[np.int16]]) -> None:
        ...
        if self._is_full_local_mode:
            ...
            # Optional forced turn end for noisy Windows/FastRTC/WebRTC environments.
            # VAD_FORCE_END_SECONDS is intentionally read from the environment here so it can
            # be changed in .env without modifying config.py.
            try:
                import os
            ...
            if speech_started and not self._is_speech_active:
                ...
                # Do not restart or clear the buffer on repeated speech_started events.
                logger.debug("VAD: repeated speech started ignored")

            if self._is_speech_active:
                self._audio_buffer.append(audio_frame.tobytes())

                start_time = self._vad_force_start_time
                if (
                    not speech_ended
