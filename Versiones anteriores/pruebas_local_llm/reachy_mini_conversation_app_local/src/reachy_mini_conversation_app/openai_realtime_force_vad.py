...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    def __init__(self, deps: ToolDependencies, gradio_mode: bool = False, instance_path: Optional[str] = None):
        ...
        self._vad_force_start_time: float | None = None  # Forced VAD turn-end timer
    ...
    async def receive(self, frame: Tuple[int, NDArray[np.int16]]) -> None:
        ...
        # Patch: in some Windows/FastRTC setups, silence is not delivered reliably,
        # so the VAD detects speech start but never emits speech end. VAD_FORCE_END_SECONDS
        # closes the turn after a configurable number of seconds.
        if self._is_full_local_mode:
            ...
            try:
                ...
                force_end_seconds = float(os.getenv("VAD_FORCE_END_SECONDS", "0") or "0")
            except Exception:
                force_end_seconds = 0.0

            now = asyncio.get_running_loop().time()

            if speech_started and not self._is_speech_active:
                self._is_speech_active = True
                self._vad_force_start_time = now
                self._audio_buffer.clear()
                self.deps.movement_manager.set_listening(True)
                logger.info("VAD: speech started")
            elif speech_started and self._is_speech_active:
                # Avoid restarting/clearing the buffer on repeated speech-start events.
                logger.debug("VAD: repeated speech started ignored")

            if self._is_speech_active:
                self._audio_buffer.append(audio_frame.tobytes())

                start_time = getattr(self, "_vad_force_start_time", None)
                if (
                    not speech_ended
                    and not self._vad_processing
                    and force_end_seconds > 0
                    and start_time is not None
                    and (now - start_time) >= force_end_seconds
                ):
                    speech_ended = True
                    logger.info("VAD: force speech ended after %.2fs", now - start_time)

            if speech_ended and not self._vad_processing:
                self._vad_processing = True
                self._is_speech_active = False
                self._vad_force_start_time = None
                self.deps.movement_manager.set_listening(False)
                ...
                # Reset LocalVAD internal state after a forced end so the next utterance
                # can start cleanly even if WebRTC did not deliver silence frames.
                for attr, value in (
                    ("is_speaking", False),
                    ...
                    try:
                        setattr(self._local_vad, attr, value)
                    except Exception:
                        pass
