...
try {
    LogLine "============================================================"
    LogLine "Ahootsa 5.0.41 - version autonoma estable"
    LogLine "============================================================"
    LogLine "Root:       $Root"
    ...
    LogLine "[INFO] Aplicando parche autónomo 5.0.41 al paquete instalado..."
    $Patch = Join-Path $Root "tools\patch_ahootsa_5_0_41.py"
    & $Py $Patch 2>&1 | Tee-Object -FilePath $RuntimeLog -Append
    if ($LASTEXITCODE -ne 0) { throw "No se pudo aplicar el parche 5.0.41." }

    # Arranque daemon en proceso aparte.
    $Tmp = Join-Path $Root ".tmp"
    New-Item -ItemType Directory -Force -Path $Tmp | Out-Null
    $DaemonScript = Join-Path $Tmp "start_daemon_5_0_41_$Session.ps1"
    @"
`$env:AHOOTSA_VERSION = '5.0.41'
`$env:AHOOTSA_LOG_ROOT = '$LogRoot'
`$env:AHOOTSA_CAMERA_DIR = '$CameraDir'
...
"@ | Set-Content -Encoding UTF8 -LiteralPath $DaemonScript
    ...
    LogLine "[INFO] Prueba ahora el panel flotante Ahootsa 5.0.41 o abre /camera/page."
} catch {
