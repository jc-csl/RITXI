...
import os
# AHOOTSA_5_0_10_LOGGING
try:
    from ahootsa_logging import log_event, timed
except Exception:
...
def _log(step: str, **data: object) -> None:
    try:
        from ahootsa_logging import log_runtime
        log_runtime(step, **data)
    except Exception:
        try:
            log_dir = Path(os.getenv("AHOOTSA_LOG_DIR", r"D:\RITXI\logs"))
            log_dir.mkdir(parents=True, exist_ok=True)
            sid = os.getenv("AHOOTSA_SESSION_ID", datetime.now().strftime("%Y%m%d_%H%M%S"))
            log_file = Path(os.getenv("AHOOTSA_LOG_FILE_RUNTIME", str(log_dir / f"ahootsa5_{sid}_runtime.log")))
            extra = ""
            if data:
...
def configure_ahootsa_realtime_ollama_environment(instance_path: Path) -> None:
    ...
    if os.getenv("AHOOTSA_RUNTIME_PROFILE_COPY", "1").strip().lower() in {"1", "true", "yes", "on"}:
        try:
    ...
    os.environ["AHOOTSA_LOG_DIR"] = os.getenv("AHOOTSA_LOG_DIR", r"D:\\RITXI\\logs")

    # AHOOTSA_5_0_4_IDENTITY_LANGUAGE_ENV
    os.environ["AHOOTSA_LANGUAGE"] = "es"
    os.environ["REACHY_MINI_LANGUAGE"] = "es"
    os.environ["APP_LANGUAGE"] = "es"
    os.environ["LANGUAGE"] = "es"
    os.environ["OUTPUT_LANGUAGE"] = "es"
    os.environ["SYSTEM_LANGUAGE"] = "es"
    os.environ["REALTIME_TRANSCRIPTION_LANGUAGE"] = "es"
    os.environ["TRANSCRIPTION_LANGUAGE"] = "es"
    os.environ["AHOOTSA_GREETING"] = "¡Hola! Soy Ahootsa. Estoy lista para ayudarte. ¿Qué quieres hacer?"
    os.environ["REACHY_MINI_GREETING"] = "¡Hola! Soy Ahootsa. Estoy lista para ayudarte. ¿Qué quieres hacer?"
    os.environ["INITIAL_GREETING"] = "¡Hola! Soy Ahootsa. Estoy lista para ayudarte. ¿Qué quieres hacer?"
    os.environ["SYSTEM_PROMPT_EXTRA"] = (
        "Tu nombre es Ahootsa. No eres Reachy Mini. "
        "Habla siempre en castellano. Saluda diciendo: "
    ...
    os.environ["AHOOTSA_RUNTIME_PROFILE_COPY"] = "1"
    ...
    os.environ.setdefault("AHOOTSA_DANCES_LIBRARY_DIR", r"D:\\RITXI\\reachy-mini-dances-library")
    ...
    os.environ.setdefault("AHOOTSA_POST_PLAY_WAIT_SECONDS", "3.0")
...
def _force_official_app_identity_config(args: object | None = None) -> None:
    """Best-effort patch for official conversation app defaults.

    Some daemon-web launches start the official engine with profile=default,
    default English greeting and the public "Reachy Mini" identity. This function
    overwrites common argument/config names before run().
    """
    profile_attrs = (
        "profile",
        "profile_name",
    ...
    for obj_name, obj in (("args", args),):
        if obj is None:
            continue
        for attr in profile_attrs:
            if hasattr(obj, attr):
                try:
                    setattr(obj, attr, PROFILE_NAME)
                    _log("arg_profile_forced", attr=attr, value=PROFILE_NAME)
                except Exception as exc:
                    _log("arg_profile_force_failed", attr=attr, error=repr(exc))
        for attr in identity_attrs:
            if hasattr(obj, attr):
                try:
                    setattr(obj, attr, "Ahootsa")
                    _log("arg_identity_forced", attr=attr, value="Ahootsa")
                except Exception as exc:
                    _log("arg_identity_force_failed", attr=attr, error=repr(exc))
        for attr in lang_attrs:
            if hasattr(obj, attr):
                try:
                    setattr(obj, attr, "es")
                    _log("arg_language_forced", attr=attr, value="es")
                except Exception as exc:
                    _log("arg_language_force_failed", attr=attr, error=repr(exc))
        for attr in greeting_attrs:
            if hasattr(obj, attr):
                try:
                    setattr(obj, attr, os.environ["AHOOTSA_GREETING"])
                    _log("arg_greeting_forced", attr=attr)
                except Exception as exc:
                    _log("arg_greeting_force_failed", attr=attr, error=repr(exc))

    try:
        import reachy_mini_conversation_app.config as cfg
        for attr in profile_attrs + ("DEFAULT_PROFILE", "PROFILE", "CURRENT_PROFILE"):
            if hasattr(cfg, attr):
                try:
                    setattr(cfg, attr, PROFILE_NAME)
                    _log("config_profile_forced", attr=attr, value=PROFILE_NAME)
                except Exception as exc:
                    _log("config_profile_force_failed", attr=attr, error=repr(exc))
        for attr in identity_attrs + ("ASSISTANT_NAME", "ROBOT_NAME", "PROJECT_NAME"):
            if hasattr(cfg, attr):
                try:
                    setattr(cfg, attr, "Ahootsa")
                    _log("config_identity_forced", attr=attr, value="Ahootsa")
                except Exception as exc:
                    _log("config_identity_force_failed", attr=attr, error=repr(exc))
        for attr in lang_attrs + ("LANGUAGE", "LOCALE"):
            if hasattr(cfg, attr):
                try:
                    setattr(cfg, attr, "es")
                    _log("config_language_forced", attr=attr, value="es")
                except Exception as exc:
                    _log("config_language_force_failed", attr=attr, error=repr(exc))
        for attr in greeting_attrs + ("GREETING", "INITIAL_GREETING", "WELCOME_MESSAGE", "INITIAL_MESSAGE"):
            if hasattr(cfg, attr):
                try:
                    setattr(cfg, attr, os.environ["AHOOTSA_GREETING"])
                    _log("config_greeting_forced", attr=attr)
                except Exception as exc:
                    _log("config_greeting_force_failed", attr=attr, error=repr(exc))
    except Exception as exc:
        _log("official_config_identity_patch_failed", error=repr(exc))
...
class AhootsaRealtimeOllamaApp(ReachyMiniApp):  # type: ignore[misc]
    ...
    def run(self, reachy_mini: ReachyMini, stop_event: threading.Event) -> None:
        ...
        args, _ = parse_args()
        log_event("app.parse_args", args=repr(args))
        _force_official_app_identity_config(args)
