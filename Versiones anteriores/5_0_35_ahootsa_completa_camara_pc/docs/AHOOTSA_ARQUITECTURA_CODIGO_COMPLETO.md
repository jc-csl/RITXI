...
**Versión documental:** 5.0.33 / actualización 5.0.34-doc  
**Fecha:** 2026-07-08  
**Contexto:** Ahootsa 5.0.25 como base instalada, con correcciones acumuladas 5.0.27–5.0.33: endpoints de compatibilidad, logs por ejecución, lanzadores PowerShell corregidos, bloqueo de audio Windows/navegador y documentación gráfica por paneles.  
...
## 0. Estado actual de la documentación

Este documento queda actualizado para recoger el estado real trabajado hasta la **corrección 5.0.33**. La base funcional sigue siendo la carpeta original:

```text
D:\RITXI\5_0_34_ahootsa_completa_consolidada_b
```

Las versiones 5.0.27 a 5.0.33 no sustituyen conceptualmente a la 5.0.25, sino que actúan como **parches correctivos acumulados** sobre la instalación existente dentro del entorno de Reachy Mini Control y sobre los scripts PowerShell de la carpeta local.

Resumen de cambios acumulados:

| Versión | Finalidad | Archivos o zonas afectadas |
|---|---|---|
| 5.0.27 | Añadir endpoints de compatibilidad sin depender de encontrar literalmente `app = FastAPI(...)`. | `ahootsa_realtime_ollama_desktop_app\__init__.py`, bootstrap FastAPI/Starlette. |
| 5.0.28 | Evitar que el arranque falle si `pantalla.log` está bloqueado. | Scripts PowerShell con escritura segura de logs. |
| 5.0.29 | Bloquear audio de Windows/navegador en actividades. | Frontend/JS, bloqueo de `speechSynthesis`. |
| 5.0.30 | Corregir wrapper PowerShell que pasaba mal `-InstallMujoco`. | `LANZAR_5_0_30_AHOOTSA_MUJOCO_WEB.ps1`. |
| 5.0.31 | Crear logs por ejecución y reconstruir script de espera. | `ESPERAR_5_BACKEND_REALTIME_LISTO.ps1`, resumen de última ejecución. |
| 5.0.32 | Reparar errores de `param(...)` en el lanzador principal. | `LANZAR_5_AHOOTSA_MUJOCO_WEB.ps1` y wrapper seguro. |
| 5.0.33 | Cortar también TTS local de Windows desde Python. | Bloqueo de `pyttsx3`, SAPI/`SAPI.SpVoice` y voz navegador. |

La regla de audio vigente es:

```text
Solo debe hablar Ahootsa.
No debe sonar voz Windows, navegador, pyttsx3 ni SAPI.
```

La regla de logs vigente es:

```text
Cada ejecución debe generar un identificador de sesión nuevo con timestamp.
El último resumen debe contener solo información útil para depurar la ejecución más reciente.
```
...
Root:    D:\RITXI\5_0_34_ahootsa_completa_consolidada_b
Logs:    D:\RITXI\logs
...
En 5.0.25 se simplificaron logs, y desde las correcciones 5.0.31/5.0.32 se refuerza que cada ejecución use un timestamp nuevo. La salida esperada por sesión es:
...
### 10.4. Endpoints de compatibilidad actuales

Los endpoints añadidos por compatibilidad evitan que el frontend o Desktop Control genere errores 404 al consultar estado, micro o voz. No todos representan una funcionalidad real completa; algunos son stubs seguros.

| Endpoint | Respuesta esperada | Motivo |
|---|---|---|
| `/status` | `ok: true`, estado de backend | Permite saber que la app está viva. |
| `/mic` | estado del micrófono o stub seguro | Evita 404 cuando el frontend pregunta por micro. |
| `/voices/current` | voz actual o voz de sistema | Evita 404 y permite mostrar estado de voz. |
| `/voices` | lista de voces o lista mínima | Compatibilidad con paneles de voz. |

Si se implementa control real de micrófono o voces, estos endpoints deben dejar de ser solo compatibilidad y pasar a consultar el módulo real correspondiente.
...
## 19.bis. Cambios acumulados posteriores a 5.0.27

### 19.bis.1. Corrección 5.0.28: logs robustos
...
- varios scripts intentaban escribir a la vez en el mismo `pantalla.log`;
- PowerShell abría el archivo para escritura sin tolerancia a bloqueo;
- cuando el fichero estaba ocupado por otro proceso, el arranque se llenaba de errores.

Solución aplicada:

- introducir una función segura de escritura, por ejemplo `Write-AhootsaSafeLog`;
- sustituir escrituras directas con `Add-Content` en scripts críticos;
- si el log está bloqueado, no se debe romper la ejecución;
- el log de pantalla debe ser informativo, no un punto único de fallo.

Regla para futuras modificaciones:

```powershell
# Evitar esto en scripts de arranque compartidos:
$Line | Add-Content -Encoding UTF8 -LiteralPath $Log

# Preferir una función segura con try/catch y fallback:
Write-AhootsaSafeLog -Path $Log -Line $Line
```

### 19.bis.2. Corrección 5.0.29: audio único Ahootsa

El problema observado fue que en el juego de parejas y otras actividades sonaban simultáneamente:

```text
voz Ahootsa + voz Windows/navegador
```

La primera corrección bloqueó la síntesis de voz del navegador:
...
- impedir que las actividades usen la voz del navegador;
- mantener solamente el canal de voz propio de Ahootsa;
- evitar duplicidad de audio en actividades educativas.

### 19.bis.3. Corrección 5.0.30: wrapper PowerShell
...
- un wrapper pasaba el parámetro `-InstallMujoco` como si fuera un valor textual;
- en PowerShell, un `SwitchParameter` no se debe pasar como string salvo que se use correctamente la sintaxis booleana.

Regla correcta:

```powershell
# Correcto: solo pasar el switch si se ha solicitado
if ($InstallMujoco) {
    & $PatchScript -InstallMujoco
} else {
    & $PatchScript
}
```

### 19.bis.4. Corrección 5.0.31: logs por ejecución y script de espera

Objetivo solicitado:

```text
almacenar cada ejecución del programa en un archivo nuevo añadiendo un timestamp
para que el último log tenga solo lo necesario para corregir el código de la última ejecución
```
...
- `pantalla.log`: lo que se ve en consola y mensajes de alto nivel;
- `eventos.jsonl`: eventos estructurados para analizar comportamiento;
- `runtime.log`: salida del daemon, backend, tracebacks y errores técnicos;
- `ULTIMA_EJECUCION_AHOOTSA_INFO.txt`: puntero a la sesión más reciente;
- `ULTIMA_EJECUCION_AHOOTSA_CORRECCION.log`: resumen filtrado con errores útiles.

El resumen de corrección debe priorizar líneas que contengan:
...
### 19.bis.5. Corrección 5.0.32: `param(...)` en PowerShell
...
En PowerShell, el bloque `param(...)` debe aparecer al inicio efectivo del script, antes de código ejecutable. Puede haber comentarios o atributos, pero no instrucciones previas.
...
Regla para futuras correcciones automáticas:

- nunca insertar funciones, banners o variables antes de `param(...)`;
- si se parchea un script, conservar `param(...)` como primer bloque lógico;
- si no se puede reparar con seguridad, crear un wrapper nuevo en vez de reescribir el lanzador original.

### 19.bis.6. Corrección 5.0.33: eliminación fuerte de audio Windows

La corrección 5.0.29 bloqueaba el audio del navegador, pero se detectó que seguía sonando voz Windows en paralelo. Por eso la 5.0.33 amplía el bloqueo a TTS local de Python/Windows.

Canales bloqueados:

| Canal | Tecnología | Estado deseado |
|---|---|---|
| Voz navegador | `speechSynthesis`, `SpeechSynthesisUtterance` | Bloqueada. |
| Voz Python local | `pyttsx3` | Bloqueada. |
| Voz Windows SAPI | `SAPI.SpVoice` | Bloqueada. |
| Audio Ahootsa/OpenAI/Realtime | canal propio de la app | Permitido. |

La intención no es dejar el sistema sin sonido. La intención es evitar que dos motores TTS hablen a la vez.

Regla funcional actual:

```text
En actividades, juegos, parejas, chat con robot y cualquier pantalla interactiva,
solo debe sonar la voz principal de Ahootsa.
```

Pruebas recomendadas:

1. Abrir actividad de parejas.
2. Pulsar una ficha con audio.
3. Confirmar que no habla Windows.
4. Confirmar que Ahootsa sigue hablando.
5. Probar otra actividad diferente para verificar que la regla es global.

Si sigue sonando Windows:

- cerrar por completo Desktop Control;
- cerrar ventanas de navegador embebido si quedan abiertas;
- relanzar con el wrapper más reciente;
- revisar si alguna actividad reproduce audio grabado o llama a un endpoint distinto de TTS.

## 19.ter. Recursos gráficos del proyecto Ahootsa

Durante la actualización se generaron piezas visuales para explicar el proyecto. Estas imágenes sirven para documentación, presentación o memoria funcional.

### 19.ter.1. Infografía global

Archivo actual de referencia global:

```text
/mnt/data/a_large_infographic_style_presentation_poster_with.png
```
...
- PERSONALIZACIÓN;
- INTERACCIONES;
- IA LOCAL RAG + MEMORIA;
- ESCENARIOS DE USO;
- EJEMPLOS DE INTERACCIÓN;
- FUTURO;
- ARQUITECTURA RESUMIDA.

Correcciones aplicadas a la infografía global:

- títulos sin numeración;
- títulos en mayúsculas;
- color fucsia corporativo;
- sustitución de fútbol por música en el texto de ejemplo;
- sustitución del balón de fútbol por balón de baloncesto;
- `Más recursos` en lugar de `Más recursos educativos`;
- eliminación de círculos rosas de revisión;
- cajas independientes con separación gris;
- línea inferior visible en la caja de arquitectura resumida;
- en IA local se eliminó el paréntesis de `(usuaria)`, quedando `Consulta del usuario`.

### 19.ter.2. Paneles separados en resolución completa

Se generaron paneles individuales para usarlos como diapositivas o como elementos independientes:

```text
objetivo_de_un_asistente_inteligente.png

diagrama_de_arquitectura_interactiva.png

componentes_de_tecnología_interactiva.png

sensores_y_actuadores_del_robot.png

diagrama_de_personalización_interactiva.png

interacciones_y_comunicación_con_robot.png

diagrama_de_ia_local_y_conectividad.png

escenarios_de_uso_con_reachy.png

ejemplos_de_interacción_educativa.png

futuro_y_tecnología_innovadora.png

arquitectura_de_interacción_inteligente.png
```

Estos paneles no sustituyen al código, pero ayudan a entender las capas funcionales del sistema.

### 19.ter.3. Relación entre imagen y arquitectura técnica

| Panel | Relación con código |
|---|---|
| OBJETIVO | Define la finalidad funcional: comunicación, interacción social, aprendizaje, motivación y personalización. |
| ARQUITECTURA | Representa usuario, robot, portátil con IA local, RAG/memoria y perfiles personalizados. |
| COMPONENTES | Agrupa conversación, movimiento, apoyo visual, memoria y base documental. |
| SENSORES Y ACTUADORES | Relaciona entrada/salida física: micrófonos, presencia, movimiento, altavoz, LEDs y gestos. |
| PERSONALIZACIÓN | Conecta perfiles, ritmo, intereses, objetivos, refuerzo y memoria de avances. |
| INTERACCIONES | Explica el flujo escucha → comprende → responde → apoya → reconduce. |
| IA LOCAL | Relaciona Ollama, RAG, embeddings, perfiles y privacidad local. |
| ESCENARIOS DE USO | Sitúa la app en terapia/apoyo individual, grupos reducidos y actividades guiadas. |
| EJEMPLOS DE INTERACCIÓN | Ilustra actividades como aficiones, música, quiz y propuesta de nuevas tareas. |
| FUTURO | Recoge sensores, panel de seguimiento, explicabilidad y más recursos. |
| ARQUITECTURA RESUMIDA | Resume el flujo completo persona usuaria → Reachy → portátil IA → motor conversacional → voz/movimiento/contenido. |

## 19.quater. Guía rápida para tocar código sin romper la instalación

### 19.quater.1. Antes de cambiar nada

1. Identificar si el cambio pertenece al frontend, al backend Python, al launcher PowerShell o a perfiles.
2. Hacer copia del archivo exacto que se va a tocar.
3. Ejecutar diagnóstico previo.
4. Confirmar qué versión correctiva está aplicada.
5. Guardar el log de la última ejecución.

### 19.quater.2. Cambios de frontend

Tocar frontend cuando el problema sea:

- botones que no responden;
- actividades visuales;
- textos de pantalla;
- duplicidad por `speechSynthesis`;
- layout, tarjetas, paneles o juegos;
...
### 19.quater.3. Cambios de backend Python

Tocar backend cuando el problema sea:

- endpoint 404 o 500;
- error `ModuleNotFoundError`;
- integración con Ollama;
- memoria/RAG;
- llamada al robot o MuJoCo;
- bloqueo de `pyttsx3` o SAPI;
...
### 19.quater.4. Cambios de PowerShell

Tocar PowerShell cuando el problema sea:
...
```text
1. param(...) siempre al principio.
2. No usar Add-Content directo contra logs compartidos.
3. Crear timestamps nuevos por ejecución.
4. No pasar switch parameters como strings.
5. No instalar dependencias en un Python distinto de apps_venv.
```

### 19.quater.5. Cambios de perfiles

Tocar perfiles cuando el problema sea:
...
apps_venv\Lib\site-packages\ahootsa_realtime_ollama_desktop_app\profiles\...
```

Ejemplo de archivo relevante:
