...
MARKER_BEGIN = "# >>> AHOOTSA_5_0_42_AUTONOMA_BEGIN"
MARKER_END = "# <<< AHOOTSA_5_0_42_AUTONOMA_END"

BOOTSTRAP = r"""
# >>> AHOOTSA_5_0_42_AUTONOMA_BEGIN
# Parche autónomo Ahootsa 5.0.42: endpoints, Ollama/HF local, cámara PC, audio Windows bloqueado.
try:
    ...
    def _ahootsa_41_log(msg):
        try:
            ...
            with open(str(_apathlib.Path(root) / 'ahootsa_5_0_42_patch.log'), 'a', encoding='utf-8') as f:
    ...
    async def _ahootsa_41_status():
        return {'ok': True, 'status': 'running', 'app': 'Ahootsa', 'version_patch': '5.0.42', 'provider': _aos.environ.get('AHOOTSA_LLM_PROVIDER','ollama')}
...
<body><h1>Cámara PC - Ahootsa 5.0.42</h1><p>Esta prueba usa la cámara del ordenador mediante permiso del navegador. No activa el micrófono.</p>
...
<script id="ahootsa-5-0-42-panel">
(function(){
  if(window.__AHOOTSA_5_0_42_PANEL__) return; window.__AHOOTSA_5_0_42_PANEL__=true;
  try{
...
'''
    ...
    def _ahootsa_41_patch_app(app):
        try:
            if getattr(app, '_ahootsa_5_0_42_patched', False):
                return app
            setattr(app, '_ahootsa_5_0_42_patched', True)
            ...
            if hasattr(app, 'middleware') and not getattr(app, '_ahootsa_5_0_42_middleware', False):
                setattr(app, '_ahootsa_5_0_42_middleware', True)
                @app.middleware('http')
                async def _ahootsa_41_inject_panel(request, call_next):
                    ...
                    try:
                        ...
                        if 'text/html' in ctype.lower():
                            ...
                            if 'ahootsa-5-0-42-panel' not in text:
    ...
    try:
        ...
        if not getattr(_fastapi.FastAPI.__init__, '_ahootsa_5_0_42_wrapped', False):
            _new_fastapi_init._ahootsa_5_0_42_wrapped = True
    ...
    try:
        ...
        def _new_starlette_init(self, *a, **k):
            ...
            _ahootsa_41_patch_app(self)
        if not getattr(_sapp.Starlette.__init__, '_ahootsa_5_0_42_wrapped', False):
            _new_starlette_init._ahootsa_5_0_42_wrapped = True
    ...
    try:
        print('AHOOTSA_5_0_42_BOOTSTRAP_ERROR', repr(_e))
    except Exception:
        pass
# <<< AHOOTSA_5_0_42_AUTONOMA_END
"""
...
def main() -> int:
    ...
    if MARKER_BEGIN in text and MARKER_END in text:
        ...
        backup = init_path.with_suffix(init_path.suffix + '.bak_5_0_42_' + datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
        shutil.copy2(init_path, backup)
        print('BACKUP=' + str(backup))
        new = BOOTSTRAP.strip() + '\n\n' + text
    init_path.write_text(new, encoding='utf-8')
    print('PATCH_OK=5.0.42')
    return 0
