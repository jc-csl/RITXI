...
from ahootsa_realtime_ollama_desktop_app.resources import config_path
...
async def _play_reaction_now(deps: ToolDependencies, kind: str) -> dict[str, Any]:
    ...
    try:
        # 7.0.23: do not expect tools/play_emotion.py. It would collide with
...
async def _run_reaction_before_speech(deps: ToolDependencies, kind: str) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"ok": True, "kind": kind, "mode": "effect_then_spoken_feedback_7_0_23"}
    try:
        reaction_result = await _play_reaction_now(deps, kind)
        ...
        try:
            ...
            if (
                ...
                duplicate = {
                    ...
                    "matched": False,
                    "memory_reaction_mode": "effect_then_spoken_feedback_7_0_23",
            ...
            # 7.0.23: no decir "Vamos a ver..." porque la imagen ya está visible
            ...
            result = {"ok": False, "error": f"{type(exc).__name__}: {exc}", "memory_reaction_mode": "direct_response_delayed_sound_7_0_23"}
