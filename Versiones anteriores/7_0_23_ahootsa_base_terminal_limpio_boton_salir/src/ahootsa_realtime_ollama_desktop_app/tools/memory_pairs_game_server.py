...
0.4.57.2:
- Removes flickering: the frontend does not rebuild cards unless visual state changes.
- Wrong pairs stay face-up for 4 seconds.
- Duplicate miss reactions are avoided when the same pair is repeated while still visible.
...
from ahootsa_realtime_ollama_desktop_app.resources import config_path, data_path, template_path
...
def _memory_timing_config() -> dict[str, float]:
    """Lee la configuración visual del juego de parejas.

    Editable desde el panel /ahootsa > Configuración del sistema mediante
    config/memory/memory_timing_config.json. Se lee de forma dinámica para que cambiar
    reveal_seconds no obligue a reinstalar ni buscar rutas a mano.
    """
    cfg = {
        "reveal_seconds": float(os.getenv("AHOOTSA_MEMORY_REVEAL_SECONDS", "8")),
        "flip_delay_seconds": float(os.getenv("AHOOTSA_MEMORY_FLIP_DELAY_SECONDS", "0.0")),
        "refresh_interval_ms": float(os.getenv("AHOOTSA_MEMORY_REFRESH_INTERVAL_MS", "3000")),
    }
    try:
        path = config_path("memory", "memory_timing_config.json")
        if path.exists():
        ...
        pass
    cfg["reveal_seconds"] = max(1.0, min(30.0, cfg["reveal_seconds"]))
    cfg["flip_delay_seconds"] = max(0.0, min(2.0, cfg["flip_delay_seconds"]))
    cfg["refresh_interval_ms"] = max(1000.0, min(10000.0, cfg["refresh_interval_ms"]))
    return cfg


def _reveal_seconds() -> float:
    return _memory_timing_config()["reveal_seconds"]


def _flip_delay_seconds() -> float:
    return _memory_timing_config()["flip_delay_seconds"]


def _refresh_interval_ms() -> int:
    return int(_memory_timing_config()["refresh_interval_ms"])

# Compatibilidad para código externo que lea estas constantes.
REVEAL_SECONDS = _reveal_seconds()
FLIP_DELAY_SECONDS = _flip_delay_seconds()

def _memory_log_event(event: str, data: dict[str, Any] | None = None) -> None:
    try:
        import importlib.util, sys
        path = Path(__file__).resolve().with_name("ahootsa_debug_logger.py")
        name = "ahootsa_debug_logger_memory_server"
        ...
        mod.log_event(event, data or {}, tool="memory_pairs_game_server")
    except Exception:
        pass
...
def _candidate_dirs() -> list[Path]:
    # 7.0.23: datos fuera de tools/. Mantener rutas antiguas como fallback.
    here = Path(__file__).resolve().parent
    return [
        data_path("memory", "games"),
        here,
        here / "games",
        Path(__file__).resolve().parents[1] / "games",
    ]
...
def _load_html() -> str:
    for base in _candidate_template_dirs():
...
def _safe_game_id(game_id: object | None) -> str:
    ...
    if requested in games:
        return requested
    aliases = {
        "animals": "animales", "animal": "animales", "parejas": "animales", "memory": "animales", "memoria": "animales", "juego": "animales", "cartas": "animales",
...
class MemoryPairsGame:
    def __init__(self, game_id: str = DEFAULT_GAME_ID) -> None:
        ...
        self.message: str = "Debes elegir dos números del 1 al 8."
        self.last_result: str = "ready"
        self.last_move_id: int = 0
        self.last_animation: str = "🎉"
        self.last_hint_pair_id: str | None = None
        self.last_pair_signature: tuple[int, int] | None = None
        self.pending_visual_until: float = 0.0
        self.pending_visual_numbers: set[int] = set()
    ...
    def reset(self, game_id: object | None = None) -> dict[str, Any]:
        with self.lock:
            ...
            self.message = "Vamos a jugar. Debes elegir dos números del 1 al 8."
            self.last_result = "ready"
            self.last_move_id += 1
            self.last_animation = "🤖"
            self.last_hint_pair_id = None
            self.last_pair_signature = None
            self.pending_visual_until = 0.0
            self.pending_visual_numbers = set()
            _memory_log_event("memory_reset", {"game_id": self.game_id})
    ...
    def _cleanup_revealed_if_needed(self) -> None:
        if self.revealed_numbers and self.reveal_until and _now() >= self.reveal_until:
            ...
            if self.last_result == "miss":
                self.message = "No eran pareja. Inténtalo otra vez con otros dos números."
    ...
    def choose(self, first: int, second: int) -> dict[str, Any]:
        with self.lock:
            ...
            pair_signature = tuple(sorted((int(first), int(second))))
            _memory_log_event("memory_choose_start", {"first": first, "second": second, "signature": pair_signature, "game_id": self.game_id})

            # If the same wrong pair is repeated while still visible, do not
            # increment move id and do not trigger a second robot reaction.
            if (
                self.last_result == "miss"
                and self.revealed_numbers
                and self.reveal_until
                and _now() < self.reveal_until
                and pair_signature == self.last_pair_signature
            ):
                return {
                    "ok": True,
                    "result": "repeat_miss",
                    "matched": False,
                    "message_for_user": "No eran pareja. Inténtalo otra vez con otros dos números.",
                    "robot_say": "",
            ...
            if first == second:
                self.message = "Elige dos cartas diferentes. Debes elegir dos números del 1 al 8."
            ...
            if not c1 or not c2:
                self.message = f"Esos números no existen. Debes elegir dos números del 1 al 8."
                self.last_result = "invalid"
                return {"ok": False, "result": "invalid", "message_for_user": self.message, "state": self.public_state()}

            if c1["number"] in self.matched_numbers or c2["number"] in self.matched_numbers:
                self.message = "Una carta ya está emparejada. Debes elegir dos números del 1 al 8."
                self.last_result = "invalid"
                return {"ok": False, "result": "invalid", "message_for_user": self.message, "state": self.public_state()}

            self.moves += 1
            self.revealed_numbers = {c1["number"], c2["number"]}
            # 7.0.23: las cartas se muestran de inmediato en el iframe. En
            # versiones anteriores se ocultaban durante FLIP_DELAY_SECONDS y
            # el usuario veía temporalmente el número; esto daba sensación de
            # número invertido durante el giro.
            flip_delay = _flip_delay_seconds()
            reveal_seconds = _reveal_seconds()
            if flip_delay > 0:
                self.pending_visual_numbers = {c1["number"], c2["number"]}
                self.pending_visual_until = _now() + flip_delay
            else:
                self.pending_visual_numbers = set()
                self.pending_visual_until = 0.0
            self.reveal_until = _now() + reveal_seconds
            self.last_pair_signature = pair_signature
            ...
            if is_match:
                ...
                self.message = f"¡Muy bien! Es pareja: {pair['left']} y {pair['right']}. Sigue así y elige otros dos números."
                result = "match"
                ...
                if len(self.found_pair_ids) == len(self.game_data["pairs"]):
                    ...
                    self.last_animation = "🏆"
                    self.message = f"¡Felicidades! Has terminado el juego en {self.moves} movimientos. ¿Quieres volver a jugar o salir?"
                    result = "final"

                _memory_log_event("memory_choose_result", {"result": result, "first": first, "second": second, "message": self.message})
                return {
                    "ok": True, "result": result, "matched": True,
                    "left": pair["left"], "right": pair["right"],
                    "message_for_user": self.message,
                    "robot_say": self.message,
                    "state": self.public_state(),
                }
            ...
            fact = ""
            try:
                pair1 = next((p for p in self.game_data.get("pairs", []) if str(p.get("id")) == str(c1.get("pair_id"))), None)
                if pair1 and pair1.get("fact"):
                    fact = " Dato curioso: " + str(pair1.get("fact"))
            except Exception:
                fact = ""
            self.message = f"No eran pareja: {c1['label']} y {c2['label']}. Inténtalo otra vez con otros dos números.{fact}"
            _memory_log_event("memory_choose_result", {"result": "miss", "first": first, "second": second, "message": self.message})
            return {
    ...
    def hint(self) -> dict[str, Any]:
        with self.lock:
            ...
            return {
                ...
                "robot_say": msg + " Dime dos números cuando quieras intentarlo.",
                "state": self.public_state(),
            }
    ...
    def public_state(self) -> dict[str, Any]:
        with self.lock:
            ...
            pending_hidden = bool(self.pending_visual_numbers and _now() < self.pending_visual_until)
            cards = []
            for card in self.cards:
                n = card["number"]
                hide_for_intro = pending_hidden and n in self.pending_visual_numbers
                visible = (n in visible_numbers) and not hide_for_intro
                matched = (n in self.matched_numbers) and not hide_for_intro
                cards.append({
                    "number": n, "visible": visible, "matched": matched,
                    "temporary": (n in self.revealed_numbers and n not in self.matched_numbers and not hide_for_intro),
                    ...
                    # Campos de apoyo para el iframe: permiten que una carta
                    # seleccionada con ratón muestre imagen inmediatamente sin
                    # crear un segundo estado visual separado del juego por voz.
                    "peek_kind_label": card["kind_label"],
                    "peek_label": card["label"],
                    "peek_icon": card["icon"],
                })
...
class _Handler(BaseHTTPRequestHandler):
    server_version = "AhootsaMemoryVisual/0.4.37"
...
def reset_game(game_id: object | None = None) -> dict[str, Any]:
    return {"ok": True, "state": _GAME.reset(game_id), "message_for_user": "He reiniciado el memory. Debes elegir dos números del 1 al 8."}


def select_game(game_id: object) -> dict[str, Any]:
    return {"ok": True, "state": _GAME.select_game(game_id), "message_for_user": "He cambiado el memory. Debes elegir dos números del 1 al 8."}
...
def reset_current_game() -> dict[str, Any]:
    return reset_game(_GAME.game_id)
