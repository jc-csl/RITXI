...
"""Ahootsa 7.0.9 panel/API routes.
...
def _load_tool_module(module_name: str):
    ...
    import sys as _sys
    mod_name = f"ahootsa_runtime_tool_{module_name}"
...
.header{{padding:10px 14px;background:#17213a;color:white;display:flex;gap:10px;align-items:center;justify-content:space-between}}
...
.board{{padding:14px;display:grid;grid-template-columns:repeat(4,1fr);gap:12px;max-width:980px;margin:0 auto}}
.card{{height:145px;border:0;border-radius:18px;background:linear-gradient(180deg,#1763c8,#0f4d9f);color:white;font-size:64px;font-weight:900;box-shadow:0 10px 22px #0002;cursor:pointer}}
.card.open,.card.matched{{background:linear-gradient(180deg,#ffe8a9,#ffd47a);color:#17213a;font-size:22px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;padding:8px}}
.card .ico{{font-size:48px}} .kind{{font-size:12px;text-transform:uppercase;color:#5b6475;font-weight:800;letter-spacing:.08em}}
.msg{{text-align:center;font-size:20px;font-weight:800;padding:8px}} .small{{font-size:12px;color:#d7dbea}}
@media(max-width:780px){{.board{{grid-template-columns:repeat(2,1fr)}}.card{{height:120px}}}}
</style></head><body>
...
function cardHtml(c){{const open=c.visible||c.matched; const cls=open?'card open'+(c.matched?' matched':''):'card'; return `<button class="${{cls}}" onclick="pick(${{c.number}})" ${{c.matched?'disabled':''}}>${{open?`<div class="ico">${{c.icon||''}}</div><div>${{c.label||''}}</div><div class="kind">${{c.kind_label||''}}</div>`:c.number}}</button>`;}}
async function refresh(){{try{{const data=await j('/memory/state'); const st=data.state||data; document.getElementById('msg').textContent=st.message||'Elige dos números.'; document.getElementById('board').innerHTML=(st.cards||[]).map(cardHtml).join('');}}catch(e){{document.getElementById('msg').textContent='No se pudo cargar el juego: '+e;}}}}
...
async function pick(n){{selected.push(n); if(selected.length<2)return; const a=selected[0],b=selected[1]; selected=[]; await j('/memory/choose?first='+a+'&second='+b); await refresh();}}
(async()=>{{await loadGames(); if({reset_js}) await resetGame(); else await j('/memory/reset?game_id={safe_game}'); await refresh(); setInterval(refresh,900);}})();
</script></body></html>"""
...
<div class='header'><strong>Ahootsa <b>7.0.9</b></strong><span>Conversación principal: Hugging Face Realtime oficial · Ollama auxiliar: {_ollama_model()}</span></div>
...
<input id='configFilter' placeholder='Filtrar: perfil, memory, tools, animales...' oninput='fillConfigSelect()'/>
...
<div class='card'><h3>JUEGO DE PAREJAS</h3><p class='small'>Versión integrada en el mismo puerto 7860. No abre una ventana externa ni usa el puerto 7870.</p>
...
<button class='secondary' onclick='openMemory()'>Abrir / reiniciar juego</button>
<iframe id='memoryFrame' style='width:100%;height:360px;border:1px solid #d7dbea;border-radius:12px;background:#eef2f7'></iframe>
</div>
...
function openMemory(){{const gid=document.getElementById('memoryGame').value||'animales';document.getElementById('memoryFrame').src='/memory/page?reset=1&game_id='+encodeURIComponent(gid);}}
