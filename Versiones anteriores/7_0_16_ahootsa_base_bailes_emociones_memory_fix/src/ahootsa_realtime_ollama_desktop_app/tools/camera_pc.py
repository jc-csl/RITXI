...
def _capture_with_cv2(camera_index: int, warmup_frames: int) -> dict[str, Any]:
    ...
    for backend in backends:
        try:
            ...
            try:
                cap.set(cv2.CAP_PROP_FRAME_WIDTH, int(os.getenv("AHOOTSA_CAMERA_PC_WIDTH", "960")))
                cap.set(cv2.CAP_PROP_FRAME_HEIGHT, int(os.getenv("AHOOTSA_CAMERA_PC_HEIGHT", "540")))
            except Exception:
                pass
