...
$Out = Join-Path $Logs "AHOOTSA_DIAGNOSTICO_DIRECTO_7_0_16_$Stamp.log"
$Py = Join-Path $AppVenv "Scripts\python.exe"
"Ahootsa diagnóstico directo 7.0.16" | Tee-Object -FilePath $Out
"Python: $Py" | Tee-Object -FilePath $Out -Append
if (-not (Test-Path -LiteralPath $Py)) {
    "ERROR: No encuentro python.exe en $Py" | Tee-Object -FilePath $Out -Append
    exit 1
}
$Tmp = Join-Path $env:TEMP "ahootsa_diag_direct_7_0_16.py"
@'
...
print("\n=== COMPROBACIÓN DE CÓDIGO PATCH 7.0.16 ===")
...
try:
    ...
    for text in ["baile uno", "baile dos", "baile tres", "baile número dos", "saludo", "saludo ahootsa", "sludo", "sluod", "celebración", "calma", "eléctrico", "play", "olay"]:
        try:
...
& $Py $Tmp 2>&1 | Tee-Object -FilePath $Out -Append
