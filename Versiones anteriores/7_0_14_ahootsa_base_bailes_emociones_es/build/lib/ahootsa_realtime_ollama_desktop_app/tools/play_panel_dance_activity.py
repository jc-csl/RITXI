...
class PlayPanelDanceActivity(Tool):
    ...
    description = "Reproduce un baile, emoción o movimiento del panel usando nombres en español: baile uno, baile dos, baile tres, celebración, saludo, calma, eléctrico."
    needs_response = True
    parameters_schema = {"type": "object", "properties": {"activity": {"type": "string", "default": "baile uno", "description": "Nombre en español o técnico. Ejemplos: baile uno, baile dos, baile tres, celebración, saludo, calma, dance1."}, "sound": {"type": "boolean", "default": True}, "delay_before_play_seconds": {"type": "number", "default": 0.0}, "post_play_wait_seconds": {"type": "number", "default": 3.0}}, "required": []}
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if not resolved:
                ...
                result = {"ok": False, "error": "actividad_no_reconocida", "requested_activity": requested, "available_examples": examples, "message_for_user": "No he reconocido ese baile o emoción. Prueba con baile uno, baile dos, baile tres, saludo, celebración o calma."}
            ...
            if isinstance(result, dict):
                if result.get("motion_ok") or (isinstance(result.get("motion"), dict) and result["motion"].get("ok")):
                    result["ok"] = True
