# Por qué no usar Reachy Mini Desktop en este modo

En este modo se recomienda no abrir Reachy Mini Desktop.

Motivo:

```text
Reachy Mini Desktop puede gestionar el ciclo de vida del daemon.
Si detecta el daemon local, puede enviar órdenes de parada.
```
...
```text
Ahootsa conversa.
MuJoCo muestra el robot 3D.
El daemon queda abierto.
```
