# 17 — Identidad Ahootsa fuerte v0.4.57

## Problema

Ahootsa controlaba el juego de cartas, pero a veces decía que se llamaba Reachy Mini.

Eso significa que las herramientas del perfil estaban cargadas, pero la identidad conversacional seguía contaminada por instrucciones del perfil oficial.

## Corrección

```text
- Se refuerza instructions.txt con identidad prioritaria.
- Se elimina la frase conflictiva "Eres Reachy Mini trabajando como Ahootsa".
- Se fuerza greeting.txt con "Soy Ahootsa".
- Se copia la identidad Ahootsa también a perfiles fallback/default.
- Se añaden variables: AHOOTSA_NAME, ASSISTANT_NAME, ROBOT_NAME.
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\REPARAR_IDENTIDAD_AHOOTSA_TOTAL.ps1
```
