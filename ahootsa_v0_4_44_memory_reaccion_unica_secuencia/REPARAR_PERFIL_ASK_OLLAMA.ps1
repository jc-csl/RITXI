...
# v0.4.44: copia el perfil Ahootsa a rutas de perfil reconocidas y fuerza el nombre de perfil.
...
function Copy-Profile($Src, $Dst) {
    ...
    $playEmotionAudio = Join-Path $Dst "play_emotion.py"
    if (-not (Test-Path $playEmotionAudio)) {
        Fail "No se ha copiado play_emotion.py con audio en $Dst"
    }

    foreach ($gameFile in @("memory_pairs_game_server.py","start_memory_pairs_game.py","choose_memory_cards.py","reset_memory_pairs_game.py","memory_pairs_game_status.py")) {
        $gf = Join-Path $Dst $gameFile
        if (-not (Test-Path $gf)) {
            Fail "No se ha copiado $gameFile en $Dst"
        }
    }
    ...
    foreach ($toolName in @("start_memory_pairs_game","choose_memory_cards","reset_memory_pairs_game","memory_pairs_game_status")) {
        $raw = Get-Content $tools -Raw -Encoding UTF8
...
Header "Reparar perfil Ahootsa ask_ollama v0.4.44"
