...
Header "Ahootsa Realtime Ollama v0.4.45 - instalacion forzada"
...
Header "Aplicar voz Sohee por defecto"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "CAMBIAR_VOZ_AHOOTSA.ps1") -Voice Sohee


Header "Instalar audio emociones por pygame"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "INSTALAR_AUDIO_EMOCIONES_PYGAME.ps1")
if ($LASTEXITCODE -ne 0) {
    Warn "No se pudo instalar/verificar pygame. Las emociones seguirán moviéndose, pero puede faltar sonido."
}
...
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "FORZAR_VOZ_SOHEE_TOTAL.ps1")

Header "Desactivar voz Windows idle"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "DESACTIVAR_VOZ_WINDOWS_IDLE_AHOOTSA.ps1")


Header "Forzar perfil Ahootsa total"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "FORZAR_PERFIL_AHOOTSA_TOTAL.ps1")
if ($LASTEXITCODE -ne 0) {
    Warn "No se pudo verificar el forzado total del perfil Ahootsa."
}

Header "Forzar inicio en castellano y voz Sohee"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "FORZAR_INICIO_CASTELLANO_SOHEE.ps1")

Header "Diagnostico final"

powershell -ExecutionPolicy Bypass -File (Join-Path $Root "test\DIAGNOSTICAR_ASK_OLLAMA.ps1")


Header "Reparar audio emociones Ahootsa"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "REPARAR_AUDIO_EMOCIONES_AHOOTSA.ps1")
if ($LASTEXITCODE -ne 0) {
    Warn "No se pudo verificar audio de emociones. El movimiento seguirá funcionando, pero revisa test\PROBAR_AUDIO_EMOCION_HERRAMIENTA.ps1."
}

Header "Diagnostico emociones audio + movimiento"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "test\DIAGNOSTICAR_EMOCIONES_AUDIO.ps1")



Header "Reparar HTML juego parejas"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "REPARAR_HTML_JUEGO_PAREJAS.ps1")

Header "Diagnostico juego parejas animales"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "test\DIAGNOSTICAR_JUEGO_PAREJAS_ANIMALES.ps1")


Header "Reparar motor JSON juegos parejas"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "REPARAR_JUEGOS_PAREJAS_JSON.ps1")

Header "Diagnostico juegos parejas JSON"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "test\DIAGNOSTICAR_JUEGOS_PAREJAS_JSON.ps1")




Header "Reparar Memory estable sin bloqueo"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "REPARAR_MEMORY_ESTABLE_SIN_BLOQUEO.ps1")

# Header "Reparar Memory reacción única" (omitido por v0.4.45)
# powershell -ExecutionPolicy Bypass -File (Join-Path $Root "REPARAR_MEMORY_REACCION_UNICA.ps1")

Header "Reparar Memory start_server"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "REPARAR_MEMORY_START_SERVER.ps1")
...
Write-Host "4. Prueba: saluda con sonido"
