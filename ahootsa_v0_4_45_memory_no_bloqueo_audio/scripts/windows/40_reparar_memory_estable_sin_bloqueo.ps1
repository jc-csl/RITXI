# REPARAR_MEMORY_ESTABLE_SIN_BLOQUEO.ps1
# v0.4.45: repara el juego Memory si se queda parado o deja de responder al audio.
...
Header "Variables de estabilidad"
[Environment]::SetEnvironmentVariable("AHOOTSA_RUNTIME_PROFILE_COPY", "0", "User")
[Environment]::SetEnvironmentVariable("AHOOTSA_MEMORY_REACTION_ENABLED", "0", "User")
[Environment]::SetEnvironmentVariable("AHOOTSA_IDLE_REMINDER_ENABLED", "0", "User")
Info "AHOOTSA_RUNTIME_PROFILE_COPY=0"
Info "AHOOTSA_MEMORY_REACTION_ENABLED=0"
...
foreach ($t in $Targets) {
    ...
    Info "Memory estable copiado en: $t"
}

Header "Comprobación"
powershell -ExecutionPolicy Bypass -File (Join-Path $Root "test\DIAGNOSTICAR_MEMORY_NO_BLOQUEO.ps1")

Header "Final"
Write-Host "Cierra completamente Reachy Mini Desktop."
Write-Host "Vuelve a abrir Ahootsa."
Write-Host "Prueba: quiero el juego de animales; uno y tres."
Write-Host "El juego debe girar cartas y seguir escuchando audio."
