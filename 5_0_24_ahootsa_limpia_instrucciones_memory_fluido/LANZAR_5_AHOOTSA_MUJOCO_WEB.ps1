...
$Session = $env:AHOOTSA_SESSION_ID
$env:AHOOTSA_LOG_DIR = $LogRoot
[Environment]::SetEnvironmentVariable("AHOOTSA_LOG_DIR", $LogRoot, "User")

$PsLog = Join-Path $LogRoot ("ahootsa_ps_LANZAR_5_AHOOTSA_MUJOCO_WEB_" + $Session + ".log")
$JsonLog = Join-Path $LogRoot ("ahootsa_ps_events_" + $Session + ".jsonl")
...
Write-Host ""
Write-Host "============================================================"
Write-Host "Ahootsa 5.0.24 - MuJoCo web backend realtime"
Write-Host "============================================================"
...
if (Test-PortOpen $Port) {
    ...
    $DaemonLog = Join-Path $LogRoot ("ahootsa_5_mujoco_daemon_" + $Session + ".log")
    $TempScript = Join-Path $LogRoot ("start_daemon_mujoco_" + $Session + ".ps1")
    ...
    $Template = @'
# start_daemon_mujoco.ps1 generado por Ahootsa 5.0.24
$ErrorActionPreference = "Continue"
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$OutputEncoding = [System.Text.Encoding]::UTF8
...
"TranscriptLog=$DaemonTranscriptLog" | Add-Content -Encoding UTF8 -LiteralPath $DaemonLog
try { Start-Transcript -LiteralPath $DaemonTranscriptLog -Append | Out-Null } catch {}
