...
try {
    Write-Host "============================================================"
    Write-Host "Ahootsa 7.0.20 - comunicación diagnóstica y latencia"
    Write-Host "============================================================"
    ...
    if (-not $NoInstall) {
        & $Py -c "import sys, importlib.metadata as m; import ahootsa_realtime_ollama_desktop_app as a; eps=list(m.entry_points(group='reachy_mini_apps')); print('AHOOTSA_INSTALLED_VERSION', a.__version__); print('HAS_AHOOTSA_ENTRYPOINT', any(e.name=='ahootsa_realtime_ollama_app' for e in eps)); sys.exit(0 if a.__version__=='7.0.20' and any(e.name=='ahootsa_realtime_ollama_app' for e in eps) else 7)"
        if ($LASTEXITCODE -ne 0) { $NeedsInstall = $true }
    }
    if ($NeedsInstall -and -not $NoInstall) {
        Write-Host "[INFO] Instalación activa no es 7.0.20 o falta entrypoint. Instalando 7.0.20..."
        $argsInstall = @('-ExecutionPolicy','Bypass','-File',(Join-Path $Root 'INSTALAR_AHOOTSA_7_0_20.ps1'),'-AppVenv',$AppVenv)
        ...
        if ($LASTEXITCODE -ne 0) { throw "No se pudo instalar Ahootsa 7.0.20." }
        $RestartDaemon = $true
    }
    ...
    if ($LASTEXITCODE -ne 0) { Write-Host "[WARN] pygame no está disponible. Instala con INSTALAR_AHOOTSA_7_0_20.ps1." }
...
"@ | Set-Content -Encoding UTF8 -LiteralPath $DaemonScript
    ...
    Write-Host "[INFO] Arrancando app Ahootsa 7.0.20..."
    try {
        ...
        Write-Host "[INFO] Ejecuta DIAGNOSTICAR_START_APP_7_0_20.ps1 y RESUMIR_LOGS_AHOOTSA_7_0_20.ps1"
        throw
    }
