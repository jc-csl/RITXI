...
$Out = Join-Path $LogRoot "AHOOTSA_TRAZA_COMUNICACION_VOZ_7_0_20_$Stamp.log"
function Add-Line([string]$Text = "") { Add-Content -LiteralPath $Out -Encoding UTF8 -Value $Text }
Add-Line "Ahootsa traza comunicacion voz 7.0.20"
...
Add-Line "3) inicia la actividad uno"
Add-Line ""
Add-Line "El script recogera eventos recientes de D:\RITXI\logs relacionados con comunicacion y Ollama."
...
$patterns = "communication|comunicacion|list_communication|start_communication|actividades_comunicacion|ask_ollama|tool_start|tool_result|duration_ms|conversation_events"
foreach ($f in $files) {
    try {
