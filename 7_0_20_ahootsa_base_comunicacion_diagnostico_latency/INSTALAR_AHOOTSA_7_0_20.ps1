...
Write-Host "============================================================"
Write-Host "Instalando Ahootsa 7.0.20"
Write-Host "============================================================"
...
if (-not $NoClean) {
    ...
    $Cleaner = Join-Path $env:TEMP "ahootsa_clean_install_7_0_20.py"
@'
...
'@ | Set-Content -Encoding UTF8 -LiteralPath $Cleaner
    ...
    $CollisionCleaner = Join-Path $env:TEMP "ahootsa_clean_collisions_7_0_20.py"
@'
from __future__ import annotations
from pathlib import Path
import shutil
...
Write-Host "[INFO] Instalando paquete Ahootsa 7.0.20 desde esta carpeta..."
Write-Host "[INFO] Se usa --ignore-installed y limpieza previa para evitar instalaciones corruptas sin RECORD."
Invoke-PythonSafe -ArgsList @('-m','pip','install','--no-cache-dir','--ignore-installed','--no-deps',$Root) -ErrorMessage "pip install de Ahootsa 7.0.20 ha fallado" | Out-Null
...
Invoke-PythonSafe -ArgsList @('-c', "import ahootsa_realtime_ollama_desktop_app as a; print('AHOOTSA_IMPORT_OK', a.__version__); raise SystemExit(0 if a.__version__=='7.0.20' else 1)") -ErrorMessage "La versión importada no es 7.0.20" | Out-Null
...
if (-not $SkipEmotionLibraryDownload) {
    ...
    $TmpPy = Join-Path $env:TEMP 'ahootsa_download_emotions_7_0_20.py'
@'
from pathlib import Path
try:
...
Write-Host "[OK] Ahootsa 7.0.20 instalada y registrada."
