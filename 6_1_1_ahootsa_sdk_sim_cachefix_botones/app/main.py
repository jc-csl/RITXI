...
@app.get("/app.js")
async def app_js_nocache():
    text = (STATIC_DIR / "app.js").read_text(encoding="utf-8")
    return JSONResponse(
        content={"version": APP_VERSION, "js": text},
        headers={"Cache-Control": "no-store, no-cache, must-revalidate, max-age=0"},
    )


@app.get("/api/ui/version")
async def ui_version() -> Dict[str, Any]:
    return {
        "version": APP_VERSION,
        "ui": "cachefix_buttons",
        "static_app_js_exists": (STATIC_DIR / "app.js").exists(),
        "robot_endpoints": [
            "/api/robot/status",
            "/api/robot/probe",
            "/api/robot/action/saludo",
            "/api/robot/action/wiggle",
            "/api/robot/action/nod",
            "/api/robot/action/look_left_right",
            "/api/robot/action/reset",
        ],
    }
