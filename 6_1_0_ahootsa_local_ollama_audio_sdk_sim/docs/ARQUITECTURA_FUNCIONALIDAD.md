...
`app/sdk_command.py` se ejecuta con el Python instalado por Reachy Mini Control.  
Así evitamos instalar el SDK dentro del venv de Ahootsa y reducimos conflictos.
...
- `navigator.mediaDevices.getUserMedia` para permisos y medidor de audio,
- `SpeechRecognition` / `webkitSpeechRecognition` para transcribir voz.

Si el medidor se mueve pero no hay texto, el problema ya no es el micro sino el motor STT del navegador.
